...
- **URL:** https://modal.com/docs/examples/grpo_verl
- **Summary:** This example demonstrates how to train with GRPO on Modal using the verl framework. GRPO is a reinforcement learning algorithm introduced by DeepSeek, and was used to train DeepSeek R1. verl is a reinforcement learning training library that is an implementation of HybridFlow, an RLHF framework.

[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/reinforcement-learning/grpo_verl.py)

Copy page

Train a model to solve math problems using GRPO and verl

This example demonstrates how to train with [GRPO](https://arxiv.org/pdf/2402.03300)
 on Modal using the [verl](https://github.com/volcengine/verl)
 framework. GRPO is a reinforcement learning algorithm introduced by DeepSeek, and was used to train DeepSeek R1. verl is a reinforcement learning training library that is an implementation of [HybridFlow](https://arxiv.org/abs/2409.19256v2)
, an RLHF framework.
...
In this example, we’ll use reinforcement learning to train a model to solve math problems. We use the [GSM8K](https://huggingface.co/datasets/openai/gsm8k)
 dataset of math problems and a [Modal Volume](https://modal.com/docs/guide/volumes#volumes)
 to store the data.
...
In reinforcement learning, we define a reward function for the model. We can define this in a separate file, or in the same file as in this case that we then pass as an argument to verl. We use a `default` reward function for GSM8K from the [verl repo](https://github.com/volcengine/verl/blob/v0.1/verl/utils/reward_score/gsm8k.py)
...
Now, we write a Modal Function for kicking off the training run. If you wish to use Weights & Biases, as we do in this code, you’ll need to create a Weights & Biases [Secret.](https://modal.com/docs/guide/secrets#secrets)
...
You can now run the training using `modal run --detach grpo_verl.py::train`, or pass in any [additional args from the CLI](https://modal.com/docs/guide/apps#argument-parsing)
 like this `modal run --detach grpo.py::train -- trainer.total_epochs=20 actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu=16`.
...
. The weights path is as follows: `global_step_n/actor/huggingface` where n is the checkpoint you want (e.g. `global_step_5/actor/huggingface`). The `latest_checkpointed_iteration.txt` file stores the most recent checkpoint index.
...
[Train a model to solve math problems using GRPO and verl](https://modal.com/docs/examples/grpo_verl#train-a-model-to-solve-math-problems-using-grpo-and-verl)
[Setup](https://modal.com/docs/examples/grpo_verl#setup)
[Defining the image and app](https://modal.com/docs/examples/grpo_verl#defining-the-image-and-app)
[Defining the dataset](https://modal.com/docs/examples/grpo_verl#defining-the-dataset)
[Defining a reward function](https://modal.com/docs/examples/grpo_verl#defining-a-reward-function)
[Kicking off a training run](https://modal.com/docs/examples/grpo_verl#kicking-off-a-training-run)
[Performing inference on the trained model](https://modal.com/docs/examples/grpo_verl#performing-inference-on-the-trained-model)
