# Maintain a pool of warm Sandboxes that are healthy and ready to serve requests | Modal Docs

- **URL:** https://modal.com/docs/examples/sandbox_pool
- **Summary:** This example demonstrates how to build a pool of “warm” Modal Sandboxes, and deploy a Modal Web Function that lets you claim a Sandbox from the pool, getting a URL to the server running in the Sandbox.

[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/13_sandboxes/sandbox_pool.py)

Copy page

Maintain a pool of warm Sandboxes that are healthy and ready to serve requests

This example demonstrates how to build a pool of “warm” [Modal Sandboxes](https://modal.com/docs/guide/sandbox)
, and deploy a [Modal Web Function](https://modal.com/docs/guide/webhook-urls)
 that lets you claim a Sandbox from the pool, getting a URL to the server running in the Sandbox.
...
It uses a [Modal Queue](https://modal.com/docs/guide/dicts-and-queues#modal-queues)
 to store references to the warm Sandboxes, and functionality to maintain the pool by adding and removing Sandboxes, checking the current size, etc.

The pool keeps track of the time to live for each Sandbox, and will always return a Sandbox with enough time left.

Each Sandbox is configured with a [readiness probe](https://modal.com/docs/guide/sandbox#readiness-probes)
 so we can reliably wait for the server to be ready before adding it to the pool.
...
The implementation borrows from [pawalt](https://github.com/pawalt)
’s [Sandbox pool example gist](https://gist.github.com/pawalt/7a505c38bba75cafae0780a5dd40e8b8)
. 🙏
...
This function creates and adds a new Sandbox to the pool. It waits for the Sandbox’s readiness probe to pass before adding it, ensuring the server is ready to serve requests.
...
*   a Function that can be called using the Modal SDK (including from [Go or JS](https://modal.com/docs/guide/sdk-javascript-go)
    ).

The Web Function proxies to `claim_sandbox` using a `.local()` invocation, which runs in the same container without additional latency.
...
[Maintain a pool of warm Sandboxes that are healthy and ready to serve requests](https://modal.com/docs/examples/sandbox_pool#maintain-a-pool-of-warm-sandboxes-that-are-healthy-and-ready-to-serve-requests)
[Main implementation](https://modal.com/docs/examples/sandbox_pool#main-implementation)
[Health check](https://modal.com/docs/examples/sandbox_pool#health-check)
[Adding a Sandbox to the pool](https://modal.com/docs/examples/sandbox_pool#adding-a-sandbox-to-the-pool)
[Claiming a Sandbox from the pool](https://modal.com/docs/examples/sandbox_pool#claiming-a-sandbox-from-the-pool)
[Maintaining the pool](https://modal.com/docs/examples/sandbox_pool#maintaining-the-pool)
[Local commands for interacting with the pool](https://modal.com/docs/examples/sandbox_pool#local-commands-for-interacting-with-the-pool)
[Deploy the app](https://modal.com/docs/examples/sandbox_pool#deploy-the-app)
[Check the current state of the pool](https://modal.com/docs/examples/sandbox_pool#check-the-current-state-of-the-pool)
[Claiming a Sandbox from the pool and print its URL](https://modal.com/docs/examples/sandbox_pool#claiming-a-sandbox-from-the-pool-and-print-its-url)
[Run a demo of the Sandbox pool.](https://modal.com/docs/examples/sandbox_pool#run-a-demo-of-the-sandbox-pool)
> Common reference appendix (shared error/status catalog): see [../_shared-appendix.md](../_shared-appendix.md).
    python 13_sandboxes/sandbox_pool.py demo
