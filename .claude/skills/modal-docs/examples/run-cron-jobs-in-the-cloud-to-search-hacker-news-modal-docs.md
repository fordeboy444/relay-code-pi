...
- **Summary:** In this example, we use Modal to deploy a cron job that periodically queries Hacker News for new posts matching a given search term, and posts the results to Slack.
...
Our Slack bot will need access to a bot token. We can use Modal’s [Secrets](https://modal.com/secrets)
 interface to accomplish this. To quickly create a Slack bot secret, click the “Create new secret” button. Then, select the Slack secret template from the list options, and follow the instructions in the “Where to find the credentials?” panel. Name your secret `hn-bot-slack.`
...
We are going to use Algolia’s [Hacker News Search API](https://hn.algolia.com/api)
 to query for posts matching a given search term in the past X days. Let’s define our search term and query period.
...
We can now define our main entrypoint, that queries Algolia for the term, and calls `post_to_slack` on all the results. We specify a [schedule](https://modal.com/docs/guide/cron)
...
Once the job is deployed, visit the [apps page](https://modal.com/apps)
 page to see its execution history, logs and other stats.

[Run cron jobs in the cloud to search Hacker News](https://modal.com/docs/examples/hackernews_alerts#run-cron-jobs-in-the-cloud-to-search-hacker-news)
[Import and define the app](https://modal.com/docs/examples/hackernews_alerts#import-and-define-the-app)
[Defining the function and importing the secret](https://modal.com/docs/examples/hackernews_alerts#defining-the-function-and-importing-the-secret)
[Searching Hacker News](https://modal.com/docs/examples/hackernews_alerts#searching-hacker-news)
[Test running](https://modal.com/docs/examples/hackernews_alerts#test-running)
[Defining the schedule and deploying](https://modal.com/docs/examples/hackernews_alerts#defining-the-schedule-and-deploying)
