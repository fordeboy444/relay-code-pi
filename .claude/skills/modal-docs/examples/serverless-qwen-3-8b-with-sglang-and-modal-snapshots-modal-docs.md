# Serverless Qwen 3-8B with SGLang and Modal Snapshots | Modal Docs

- **URL:** https://modal.com/docs/examples/sglang_snapshot
- **Summary:** In this example, we show how to serve SGLang on Modal with ~10x faster cold starts.

[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/llm-serving/sglang_snapshot.py)
...
In this example, we show how to serve [SGLang](https://github.com/sgl-project/sglang)
 on Modal with ~10x faster cold starts.
...
The key technique is [CPU + GPU memory snapshotting](https://modal.com/docs/guide/memory-snapshot)
, which saves and restores the SGLang server directly from its in-memory state.

This adds some complexity to the deployment. If you just want to get started running a basic LLM server on Modal, see [this example](https://modal.com/docs/examples/llm_inference)
.
...
We also choose a GPU to deploy our inference server onto. We choose the [H100 GPU](https://modal.com/blog/introducing-h100)
, which offers excellent price-performance and supports 8bit floating point operations, which are the lowest precision well-supported in the relevant [GPU kernels](https://modal.com/gpu-glossary/device-software/kernel)
 across a variety of model architectures.

    N_GPUS = 1
    GPU = f"H100!:{N_GPUS}"

Actual speedups are generally less than what you get from “napkin math” based on available bandwidths — we observed a speedup of about 30% moving from one to two H100s when developing this example. We recommend [application-specific benchmarking](https://modal.com/llm-almanac/how-to-benchmark)
...
. For lower latency and faster cold starts, we pick a smaller model (8B params) in a lower precision floating point format (FP8). This reduces the amount of data that needs to be loaded [from GPU RAM into SM SRAM](https://modal.com/gpu-glossary/perf/memory-bandwidth)
...
As of version `0.5.6`, SGLang’s default kernel backend for FP8 matrix multiplications (`fp8-gemm-backend`) on Hopper [SM architecture](https://modal.com/gpu-glossary/device-hardware/streaming-multiprocessor-architecture)
...
For instance, we here set an environment variable that improves the compatibility of the [Torch Inductor compiler](https://dev-discuss.pytorch.org/t/torchinductor-a-pytorch-native-compiler-with-define-by-run-ir-and-symbolic-shapes/747)
...
*   [`@app.cls`](https://modal.com/docs/guide/lifecycle-functions)
     to define the core of our service. We attach our Image, request a GPU, attach our cache Volumes, specify the region, and configure auto-scaling. See [the reference documentation](https://modal.com/docs/reference/modal.App#cls)
     for details.
...
*   [`@modal.concurrent`](https://modal.com/docs/guide/concurrent-inputs)
     to specify how many requests our server can handle before we need to scale up.
    
*   [`@modal.enter` and `@modal.exit`](https://modal.com/docs/guide/lifecycle-functions)
     to indicate which methods of the class should be run when starting the server and shutting it down. The `enter` methods also define what code is run before memory snapshot creation (`snap=True`) and after memory snapshot restoration (`snap=False`).
...
We can set two values with the [`@modal.concurrent`](https://modal.com/docs/guide/concurrent-inputs)
 decorator. `max_inputs` should be set to the maximum number of inputs a replica can handle concurrently without internal queueing — the `max-running-requests` in SGLang. `target_inputs` can be left unset or, if the per-request latency degrades too much when handling the maximum batch size, it can be set to a lower value.
...
You can find [interactive Swagger UI docs](https://swagger.io/tools/swagger-ui/)
 at the `/docs` route of that URL, i.e. `https://your-workspace-name--example-sglang-snapshot-sglang.modal.direct/docs`. These docs describe each route and indicate the expected input and output and translate requests into `curl` commands. For simple routes, you can even send a request directly from the docs page.
...
You should observe startup improvements after a handful of cold starts (usually less than five). If you want to see the speedup during a test, we recommend heading to the deployed App in your [Modal dashboard](https://modal.com/apps)
 and manually stopping containers after they have served a request to ensure turnover.
...
[Serverless Qwen 3-8B with SGLang and Modal Snapshots](https://modal.com/docs/examples/sglang_snapshot#serverless-qwen-3-8b-with-sglang-and-modal-snapshots)
[Set up the container image](https://modal.com/docs/examples/sglang_snapshot#set-up-the-container-image)
[Loading and cacheing the model weights](https://modal.com/docs/examples/sglang_snapshot#loading-and-cacheing-the-model-weights)
[Cacheing compilation artifacts](https://modal.com/docs/examples/sglang_snapshot#cacheing-compilation-artifacts)
[Speed up cold starts with GPU snapshotting](https://modal.com/docs/examples/sglang_snapshot#speed-up-cold-starts-with-gpu-snapshotting)
[Sleeping and waking an SGLang server](https://modal.com/docs/examples/sglang_snapshot#sleeping-and-waking-an-sglang-server)
[Define the inference server and infrastructure](https://modal.com/docs/examples/sglang_snapshot#define-the-inference-server-and-infrastructure)
[Determining autoscaling policy with @modal.concurrent](https://modal.com/docs/examples/sglang_snapshot#determining-autoscaling-policy-with-modalconcurrent)
[Controlling container lifecycles with @modal.enter](https://modal.com/docs/examples/sglang_snapshot#controlling-container-lifecycles-with-modalenter)
[Deploy the server](https://modal.com/docs/examples/sglang_snapshot#deploy-the-server)
[Interact with the server](https://modal.com/docs/examples/sglang_snapshot#interact-with-the-server)
[Test the server](https://modal.com/docs/examples/sglang_snapshot#test-the-server)
[Test memory snapshotting](https://modal.com/docs/examples/sglang_snapshot#test-memory-snapshotting)
> Common reference appendix (shared error/status catalog): see [../_shared-appendix.md](../_shared-appendix.md).
    python 06_gpu_and_ml/llm-serving/sglang_snapshot.py
