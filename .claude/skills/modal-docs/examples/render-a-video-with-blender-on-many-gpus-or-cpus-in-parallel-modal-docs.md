# Render a video with Blender on many GPUs or CPUs in parallel | Modal Docs

- **URL:** https://modal.com/docs/examples/blender_video
- **Summary:** This example shows how you can render an animated 3D scene using Blender’s Python interface.
...
This example shows how you can render an animated 3D scene using [Blender](https://www.blender.org/)
’s Python interface.
...
We can configure the rendering process to use GPU acceleration with NVIDIA CUDA. We select the [Cycles rendering engine](https://www.cycles-renderer.org/)
, which is compatible with CUDA, and then activate the GPU.
...
We give the `local_entrypoint` two parameters to control the render — the number of frames to render and how many frames to skip. These demonstrate a basic pattern for controlling Functions on Modal from a local client.
...
[Render a video with Blender on many GPUs or CPUs in parallel](https://modal.com/docs/examples/blender_video#render-a-video-with-blender-on-many-gpus-or-cpus-in-parallel)
[Defining a Modal app](https://modal.com/docs/examples/blender_video#defining-a-modal-app)
[Rendering a single frame](https://modal.com/docs/examples/blender_video#rendering-a-single-frame)
[Rendering with acceleration](https://modal.com/docs/examples/blender_video#rendering-with-acceleration)
[Combining frames into a video](https://modal.com/docs/examples/blender_video#combining-frames-into-a-video)
[Rendering in parallel in the cloud from the comfort of the command line](https://modal.com/docs/examples/blender_video#rendering-in-parallel-in-the-cloud-from-the-comfort-of-the-command-line)
