# Publish interactive datasets with Datasette | Modal Docs

- **URL:** https://modal.com/docs/examples/cron_datasette
- **Summary:** Build and deploy an interactive movie database that automatically updates daily with the latest IMDb data. This example shows how to serve a Datasette application on Modal with millions of movie and TV show records.

[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/10_integrations/cron_datasette.py)
...
To separate database creation and maintenance from serving, we’ll need the underlying database file to be stored persistently. To achieve this we use a [Volume](https://modal.com/docs/guide/volumes)
.
...
[IMDb Datasets](https://datasets.imdbws.com/)
 are available publicly and are updated daily. We will download the title.basics.tsv.gz file which contains basic information about all titles (movies, TV shows, etc.). Since we are serving an interactive database which updates daily, we will download the files into a temporary directory and then move them to the volume to prevent downtime.
...
IMDb updates their data daily, so we set up a [scheduled](https://modal.com/docs/guide/cron)
 function to automatically refresh the database every 24 hours.
...
Hooking up the SQLite database to a Modal Function is as simple as it gets. The Modal `@asgi_app` decorator wraps a few lines of code: one `import` and a couple more that instantiate the `Datasette` instance and return its app server.
...
[Publish interactive datasets with Datasette](https://modal.com/docs/examples/cron_datasette#publish-interactive-datasets-with-datasette)
[Basic setup](https://modal.com/docs/examples/cron_datasette#basic-setup)
[Persistent dataset storage](https://modal.com/docs/examples/cron_datasette#persistent-dataset-storage)
[Getting a dataset](https://modal.com/docs/examples/cron_datasette#getting-a-dataset)
[Data processing](https://modal.com/docs/examples/cron_datasette#data-processing)
[Inserting into SQLite](https://modal.com/docs/examples/cron_datasette#inserting-into-sqlite)
[Keep it fresh](https://modal.com/docs/examples/cron_datasette#keep-it-fresh)
[Web Function](https://modal.com/docs/examples/cron_datasette#web-function)
[Publishing to the web](https://modal.com/docs/examples/cron_datasette#publishing-to-the-web)
