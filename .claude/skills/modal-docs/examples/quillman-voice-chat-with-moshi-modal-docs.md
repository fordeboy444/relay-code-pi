# QuiLLMan: Voice Chat with Moshi | Modal Docs

- **URL:** https://modal.com/docs/examples/llm-voice-chat
- **Summary:** QuiLLMan is a complete voice chat application built on Modal: you speak and the chatbot speaks back!
...
Thanks to bidirectional websocket streaming and [Opus audio compression](https://opus-codec.org/)
...
Both frontend and backend are served via a [FastAPI Server](https://fastapi.tiangolo.com/)
, which is a popular Python web framework for building REST APIs.

On Modal, a function or class method can be exposed to web traffic by decorating it with [`@app.asgi_app()`](https://modal.com/docs/sdk/py/latest/modal.asgi_app#modalasgi_app)
...
Kyutai Lab’s [Moshi](https://github.com/kyutai-labs/moshi)
...
Under the hood, Moshi uses the [Mimi](https://huggingface.co/kyutai/mimi)
 streaming encoder/decoder model to maintain an unbroken stream of audio in and out. The encoded audio is processed by a [speech-text foundation model](https://huggingface.co/kyutai/moshiko-pytorch-bf16)
...
To solve statefulness, we just spin up a new GPU per concurrent user. That’s easy with Modal!
...
To run a [development server](https://modal.com/docs/guide/webhooks#developing-with-modal-serve)
...
For efficient audio transmission, we use the [Opus codec](https://opus-codec.org/)
 to compress audio across the network. Opus recording and playback are supported by the [`opus-recorder`](https://github.com/chris-rudmin/opus-recorder)
 and [`ogg-opus-decoder`](https://github.com/eshaz/wasm-audio-decoders/tree/master/src/ogg-opus-decoder)
 libraries.
...
[QuiLLMan: Voice Chat with Moshi](https://modal.com/docs/examples/llm-voice-chat#quillman-voice-chat-with-moshi)
[Code overview](https://modal.com/docs/examples/llm-voice-chat#code-overview)
[FastAPI Server](https://modal.com/docs/examples/llm-voice-chat#fastapi-server)
[Moshi Websocket Server](https://modal.com/docs/examples/llm-voice-chat#moshi-websocket-server)
[React Frontend](https://modal.com/docs/examples/llm-voice-chat#react-frontend)
[Deploy](https://modal.com/docs/examples/llm-voice-chat#deploy)
[Steal this example](https://modal.com/docs/examples/llm-voice-chat#steal-this-example)
