...
- **URL:** https://modal.com/docs/guide/model-weights
- **Summary:** Efficiently managing the weights of large models is crucial for optimizing the build times and startup latency of many ML and AI applications.
...
Volumes are attached by including them in a dictionary that maps a path on the remote machine to a `modal.Volume` object. They look just like a normal file system, so model weights can be saved to them without adding any special code.
...
Add [Modal Secrets](https://modal.com/docs/guide/secrets)
 to access weights that require authentication.

See [below](https://modal.com/docs/guide/model-weights#storing-weights-from-the-hugging-face-hub-on-modal)
 for more on downloading from the popular Hugging Face Hub.
...
For that, you can use the `batch_upload` method of [`modal.Volume`](https://modal.com/docs/sdk/py/latest/modal.Volume)
s via the Modal Python client library:
...
Alternatively, you can upload model weights using the [`modal volume`](https://modal.com/docs/cli/latest/volume)
 CLI command:
...
If your model weights are already in cloud storage, for example in an S3 bucket, you can connect them to Modal Functions with a `CloudBucketMount`.

See [the guide](https://modal.com/docs/guide/cloud-bucket-mounts)
 for details.
...
, the private file system state that a Function sees when it starts up. The weights might be downloaded via shell commands with [`Image.run_commands`](https://modal.com/docs/guide/images)
...
, as described [above](https://modal.com/docs/guide/model-weights#storing-weights-in-a-modal-volume)
. Performance is similar for the two methods. Volumes are more flexible. Images are rebuilt when their definition changes, starting from the changed layer, which increases reproducibility for some builds but leads to unnecessary extra downloads in most cases.

Optimizing model weight reads with `@modal.enter` 

In the above code samples, weights are loaded from disk into memory each time the `inference` function is run. This isn’t so bad if inference is much slower than model loading (e.g. it is run on very large datasets) or if the model loading logic is smart enough to skip reloading.

To guarantee a particular model’s weights are only loaded once, you can use the `@modal.enter` [container lifecycle hook](https://modal.com/docs/guide/lifecycle-functions)
...
If you need to load a single but possibly different model on each container start, you can [parametrize](https://modal.com/docs/guide/parametrized-functions)
 your Modal Cls. Below, we use the `modal.parameter` syntax.
...
The [Hugging Face Hub](https://huggingface.co/models)
 has over 1,000,000 models with weights available for download.
...
[Storing model weights on Modal](https://modal.com/docs/guide/model-weights#storing-model-weights-on-modal)
[Storing weights in a Modal Volume](https://modal.com/docs/guide/model-weights#storing-weights-in-a-modal-volume)
[Saving model weights into a Modal Volume from a Modal Function](https://modal.com/docs/guide/model-weights#saving-model-weights-into-a-modal-volume-from-a-modal-function)
[Uploading model weights into a Modal Volume](https://modal.com/docs/guide/model-weights#uploading-model-weights-into-a-modal-volume)
[Mounting cloud buckets as Modal Volumes](https://modal.com/docs/guide/model-weights#mounting-cloud-buckets-as-modal-volumes)
[Reading model weights from a Modal Volume](https://modal.com/docs/guide/model-weights#reading-model-weights-from-a-modal-volume)
[Storing weights in the Modal Image](https://modal.com/docs/guide/model-weights#storing-weights-in-the-modal-image)
[Optimizing model weight reads with @modal.enter](https://modal.com/docs/guide/model-weights#optimizing-model-weight-reads-with-modalenter)
[Storing weights from the Hugging Face Hub on Modal](https://modal.com/docs/guide/model-weights#storing-weights-from-the-hugging-face-hub-on-modal)
