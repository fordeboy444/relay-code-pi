...
- **URL:** https://modal.com/docs/guide/custom-container
- **Summary:** This guide walks you through how to define a Modal Image, the environment your Modal code runs in.
...
If you have your own container image definitions, like a Dockerfile or a registry link, you can use those too! See [this guide](https://modal.com/docs/guide/existing-images)
.

This page is a high-level guide to using Modal Images. For reference documentation on the `modal.Image` object, see [this page](https://modal.com/docs/sdk/py/latest/modal.Image)
.

What are Images? 

Your code on Modal runs in _containers_. Containers are like light-weight virtual machines — container engines use [operating system tricks](https://earthly.dev/blog/chroot/)
...
By default, Modal Functions and Sandboxes run in a [Debian Linux](https://en.wikipedia.org/wiki/Debian)
...
The simplest and most common Image modification is to add a third party Python package, like [`pandas`](https://pandas.pydata.org/)
.

You can add Python packages to the environment by passing all the packages you need to the [`Image.uv_pip_install`](https://modal.com/docs/sdk/py/latest/modal.Image#uv_pip_install)
 method, which installs packages with [`uv`](https://docs.astral.sh/uv/)
:
...
You can include [Python dependency version specifiers](https://peps.python.org/pep-0508/)
, like `"torch<3"`, in the arguments. But we recommend pinning dependencies tightly, like `"torch==2.8.0"`, to improve the reproducibility and robustness of your builds.

If you run into any issues with [`Image.uv_pip_install`](https://modal.com/docs/sdk/py/latest/modal.Image#uv_pip_install)
, then you can fallback to [`Image.pip_install`](https://modal.com/docs/sdk/py/latest/modal.Image#pip_install)
 which uses standard [`pip`](https://pip.pypa.io/en/stable/user_guide/)
:
...
You can add Python code that’s importable locally to your container by providing the module name to [`Image.add_local_python_source`](https://modal.com/docs/sdk/py/latest/modal.Image#add_local_python_source)
.
...
This is intended for pure Python auxiliary modules that are part of your project and that your code imports. Third party packages should be installed via [`Image.uv_pip_install`](https://modal.com/docs/sdk/py/latest/modal.Image#uv_pip_install)
 or similar.
...
If you have a lot of Functions and a lot of Python packages, you might want to keep the imports in the global scope so that every function can use the same imports. In that case, you can use the [`Image.imports`](https://modal.com/docs/sdk/py/latest/modal.Image#imports)
 context manager:
...
Because these imports happen before a new container processes its first input, you can combine this context manager with [Memory Snapshots](https://modal.com/docs/guide/memory-snapshots)
...
You can install Linux packages with the [`apt` package manager](https://www.debian.org/doc/manuals/apt-guide/index.en.html)
 using [`Image.apt_install`](https://modal.com/docs/sdk/py/latest/modal.Image#apt_install)
:
...
You can change the environment variables that your code sees (in, e.g., [`os.environ`](https://docs.python.org/3/library/os.html#os.environ)
) by passing a dictionary to [`Image.env`](https://modal.com/docs/sdk/py/latest/modal.Image#env)
:
...
You can supply shell commands that should be executed when building the Image to [`Image.run_commands`](https://modal.com/docs/sdk/py/latest/modal.Image#run_commands)
:
...
You can run Python code as a build step using the [`Image.run_function`](https://modal.com/docs/sdk/py/latest/modal.Image#run_function)
 method.
...
For details on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights)
.

Essentially, this is equivalent to running a Modal Function and snapshotting the resulting filesystem as a new Image. Any kwargs accepted by [`@app.function`](https://modal.com/docs/sdk/py/latest/modal.App#function)
 ([`Volume`s](https://modal.com/docs/guide/volumes)
, [`Secret`s](https://modal.com/docs/guide/secrets)
, specifications of resources like [GPUs](https://modal.com/docs/guide/gpu)
) can be supplied here.

Whenever you change other features of your Image, like the base Image or the version of a Python package, the Image will automatically be rebuilt the next time it is used. This is a bit more complicated when changing the contents of functions. See the [reference documentation](https://modal.com/docs/sdk/py/latest/modal.Image#run_function)
 for details.
...
You can set the Image Builder Version for your workspace by going to your [workspace settings](https://modal.com/settings/image-builder-version)
. This page also documents the important updates in each version.

[Images](https://modal.com/docs/guide/custom-container#images)
[What are Images?](https://modal.com/docs/guide/custom-container#what-are-images)
[Add Python packages](https://modal.com/docs/guide/custom-container#add-python-packages)
[Add local files with add_local_dir and add_local_file](https://modal.com/docs/guide/custom-container#add-local-files-with-add_local_dir-and-add_local_file)
[Add local Python code with add_local_python_source](https://modal.com/docs/guide/custom-container#add-local-python-code-with-add_local_python_source)
[What if I have different Python packages locally and remotely?](https://modal.com/docs/guide/custom-container#what-if-i-have-different-python-packages-locally-and-remotely)
[Install system packages with .apt_install](https://modal.com/docs/guide/custom-container#install-system-packages-with-apt_install)
[Set environment variables with .env](https://modal.com/docs/guide/custom-container#set-environment-variables-with-env)
[Run shell commands with .run_commands](https://modal.com/docs/guide/custom-container#run-shell-commands-with-run_commands)
[Run a Python function during your build with .run_function](https://modal.com/docs/guide/custom-container#run-a-python-function-during-your-build-with-run_function)
[Attach GPUs during setup](https://modal.com/docs/guide/custom-container#attach-gpus-during-setup)
[Use mamba instead of pip with micromamba_install](https://modal.com/docs/guide/custom-container#use-mamba-instead-of-pip-with-micromamba_install)
[Image caching and rebuilds](https://modal.com/docs/guide/custom-container#image-caching-and-rebuilds)
[Image builder updates](https://modal.com/docs/guide/custom-container#image-builder-updates)

See it in action

[Define an Image for LLM inference](https://modal.com/docs/examples/llm_inference)

[Build Images that use CUDA](https://modal.com/docs/examples/install_cuda)
