...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#tldr)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#step-1-replace-your-access-token-server-action-with-two-server-actions)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#step-2-update-the-transport-wiring)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#step-3-drop-transport-level-trigger-config)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#step-4-use-clientdata-for-typed-payload-metadata)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#step-5-update-your-chatsession-persistence)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#chat-requestupgrade--same-call-faster-handoff)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#hitting-raw-urls)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#what-didn%E2%80%99t-change)
...
*   `onAction` is still defined the same way, but its semantics changed in the [May 6 prerelease](https://trigger.dev/docs/ai-chat/changelog)
     — actions are no longer turns, and `onAction` returning a `StreamTextResult` produces a model response.
...
*   Multi-tab coordination (`multiTab: true`), [pending messages / steering](https://trigger.dev/docs/ai-chat/pending-messages)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#verifying-the-migration)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#v4-5-wire-format-change)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#why-the-change)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#object-store-configuration)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#custom-transports)

Custom transports

If you’ve built your own transport (Slack bot, CLI, native app) against the [Client Protocol](https://trigger.dev/docs/ai-chat/client-protocol)
, the `ChatTaskWirePayload` shape changed:
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#hydratemessages-consumers)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#onchatstart-is-now-once-per-chat)
...
If you need per-turn setup that **does** run on continuations, move it to [`onTurnStart`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onturnstart)
 — that hook still fires on every turn, including the first turn of a continuation run.

### 

[​](https://trigger.dev/docs/ai-chat/upgrade-guide#move-chat-local-init-from-onchatstart-to-onboot)
...
Because `onChatStart` no longer fires on continuation runs, **`chat.local`** state initialized there will be missing when a continuation run starts — `run()` then crashes with `"chat.local can only be modified after initialization"`. The fix is to move per-process initialization to the new [`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)
 hook, which fires once per worker boot (initial, preloaded, AND continuation):
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#client-side-setmessages-doesn%E2%80%99t-round-trip)

Client-side `setMessages` doesn’t round-trip

The new wire makes one thing explicit that was implicit before: **mutating `useChat()`’s messages on the client doesn’t change the agent’s history.** Full-history mutations were silently overwritten by the wire’s accumulator before this release; now they aren’t even shipped. For history compaction, summarization, or branch-swap, mutate the agent’s accumulator inside `onTurnStart` using [`chat.setMessages()`](https://trigger.dev/docs/ai-chat/backend)
 or [`chat.history.set()`](https://trigger.dev/docs/ai-chat/backend#chat-history)
. The client’s `useChat` will reconcile against the next `session.out` payload.

### 

[​](https://trigger.dev/docs/ai-chat/upgrade-guide#verifying-the-v4-5-migration)
...
[​](https://trigger.dev/docs/ai-chat/upgrade-guide#reference)
...
*   [Persistence and replay](https://trigger.dev/docs/ai-chat/patterns/persistence-and-replay)
     — snapshot model end-to-end

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/mcp)
[ChangelogPre-release updates for AI chat agents.\
\
Next](https://trigger.dev/docs/ai-chat/changelog)
