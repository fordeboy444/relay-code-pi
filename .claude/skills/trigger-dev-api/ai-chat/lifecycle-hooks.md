...
`chat.agent({ ... })` accepts a set of lifecycle hooks for persisting state, validating input, transforming messages, and reacting to suspension and resumption. They fire at well-defined points in the chat agent’s lifetime. **Once per worker process (every fresh run boot):** `onBoot` → `onPreload` (preloaded runs only). **Once per chat (first message of the chat’s lifetime):** `onChatStart`. **Per-turn order:** `onValidateMessages` → `hydrateMessages` → `onChatStart` (chat’s first message only) → `onTurnStart` → `run()` → `onBeforeTurnComplete` → `onTurnComplete`. **Suspend / resume:** `onChatSuspend` fires when the run transitions from idle to suspended (waiting on the next message); `onChatResume` fires on wake. **Four scopes to keep straight:**

| Scope | Fires when | Use for |
| --- | --- | --- |
| **Process** ([`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)<br>) | Every fresh worker boots — initial, preloaded, and reactive continuation (post-cancel/crash/`endRun`/upgrade). | Initialize `chat.local`, open per-process resources, re-hydrate state from your DB on continuation. |
| **Recovery** ([`onRecoveryBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onrecoveryboot)<br>) | Continuation boot where the dead run was mid-stream — a partial assistant survives on `session.out`. | Override the smart default — drop the partial, synthesize tool results, emit a recovery banner. |
| **Chat** ([`onChatStart`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onchatstart)<br>) | First message of a chat’s lifetime. Does NOT fire on continuation runs or OOM retries. | One-time DB rows for the chat, resources tied to the chat’s lifetime. |
| **Turn** ([`onTurnStart`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onturnstart)<br>, [`onTurnComplete`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onturncomplete)<br>, etc.) | Every turn. | Persist messages, post-process responses. |

[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#task-context-ctx)

Task context (`ctx`)
----------------------------------------------------------------------------------------------

Every chat lifecycle callback and the `run` payload include `ctx`: the same run context object as `task({ run: (payload, { ctx }) => ... })`. Import the type with `import type { TaskRunContext } from "@trigger.dev/sdk"` (the `Context` export is the same type). Use `ctx` for tags, metadata, or any API that needs the full run record. The string `runId` on chat events is always `ctx.run.id` (both are provided for convenience). See [Task context (`ctx`)](https://trigger.dev/docs/ai-chat/reference#task-context-ctx)
 in the API reference. Standard [task lifecycle hooks](https://trigger.dev/docs/tasks/overview)
 such as `onWait`, `onResume`, `onComplete`, and `onFailure` are also available on `chat.agent()` with the same shapes as on a normal `task()` — but prefer the chat-specific [`onChatSuspend` / `onChatResume`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onchatsuspend--onchatresume)
 for any chat-related work. The generic hooks fire on every wait/resume (including ones the runtime uses internally for non-chat reasons); the chat-specific ones fire only at the idle-to-suspended transition you actually care about and carry full chat context.

[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)

onBoot
----------------------------------------------------------------------

Fires **once per worker process picking up the chat** — for the initial run, for preloaded runs, AND for reactive continuation runs (post-cancel, crash, `endRun`, `requestUpgrade`, OOM retry). Does NOT fire when the same run resumes from snapshot via the idle-window suspend/resume path — use [`onChatResume`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onchatsuspend--onchatresume)
 for that. This is the right place to initialize anything that lives in the JS process for the lifetime of the run: [`chat.local`](https://trigger.dev/docs/ai-chat/chat-local)
 state, [DB connections](https://trigger.dev/docs/database-connections)
, sandboxes, in-memory caches. It runs before `onPreload`, `onChatStart`, the continuation-wait branch, and any turn — so anything you set up here is available everywhere downstream.
...
| --- | --- | --- |
| `ctx` | `TaskRunContext` | Full task run context. See [reference](https://trigger.dev/docs/ai-chat/reference#task-context-ctx)<br>. |
...
Returns `{ chain?, recoveredTurns?, beforeBoot? }` — every field optional. Omitted fields fall through to the smart default. See [Recovery boot](https://trigger.dev/docs/ai-chat/patterns/recovery-boot)
 for the full guide, examples (drop partial, synthesize tool results, persist before boot), and interaction notes.

Don’t put `chat.local` initialization in `onRecoveryBoot` — use [`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)
. `onRecoveryBoot` is for recovery decisions, not per-process setup. `onBoot` fires first.

[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onpreload)

onPreload
----------------------------------------------------------------------------

Fires when a **preloaded run** starts, before any messages arrive. Use it to eagerly create chat-scoped DB rows (the Chat row, the ChatSession row) while the user is still typing — so the very first message lands fast. Preloaded runs are triggered by calling `transport.preload(chatId)` on the frontend. See [Preload](https://trigger.dev/docs/ai-chat/fast-starts#preload)
 for details. Per-process state (anything in [`chat.local`](https://trigger.dev/docs/ai-chat/chat-local)
, DB connections, etc.) belongs in [`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)
 — `onBoot` fires before `onPreload` on every fresh worker, including on continuation runs where `onPreload` never fires.
...
[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onchatstart)

onChatStart
--------------------------------------------------------------------------------

Fires **exactly once per chat**, on the very first user message of the chat’s lifetime, before `run()` executes. Use it for one-time chat-scoped setup — create the Chat DB row, mint resources tied to the chat’s lifetime. `onChatStart` does **not** fire on:
...
For per-process state that has to be initialized on every fresh worker (including continuation runs), use [`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)
. For per-turn setup, use [`onTurnStart`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onturnstart)
.

Do not initialize [`chat.local`](https://trigger.dev/docs/ai-chat/chat-local)
 here. `chat.local` is per-process state that must survive continuation runs, but `onChatStart` only fires on the chat’s very first message. Use [`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)
 instead.

The `preloaded` field tells you whether [`onPreload`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onpreload)
 already ran for this chat — useful for skipping setup work that’s already done.
...
`clientData` contains custom data from the frontend: either the `clientData` option on the transport constructor (sent with every message) or the `metadata` option on `sendMessage()` (per-message). See [Client data and metadata](https://trigger.dev/docs/ai-chat/frontend#client-data-and-metadata)
.

[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onvalidatemessages)

onValidateMessages
----------------------------------------------------------------------------------------------

Validate or transform incoming `UIMessage[]` before they are converted to model messages. Fires on turns that carry incoming messages, with the raw messages from the wire payload (after cleanup of aborted tool parts), **before** accumulation and `toModelMessages()`. Turns with no incoming messages — preload, close, and regenerate with nothing re-sent — skip it. Return the validated messages array. Throw to abort the turn with an error. This is the right place to call the AI SDK’s [`validateUIMessages`](https://ai-sdk.dev/docs/ai-sdk-ui/chatbot-message-persistence#validating-messages-on-the-server)
 to catch malformed messages from storage or untrusted input before they reach the model, especially useful when persisting conversations to a database where tool schemas may drift between deploys.
...
[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#hydratemessages)
...
| `incomingMessages` | `UIMessage[]` | Validated incoming messages for this turn. Usually 0-or-1 (empty for actions, regenerates, and continuations; one element for normal `submit-message` and tool-approval responses). On a [Head Start](https://trigger.dev/docs/ai-chat/fast-starts#with-hydratemessages)<br> first turn, this can contain the route handler’s first-turn history. |
...
`upsertIncomingMessage` (exported from `@trigger.dev/sdk/ai`) handles the three cases that matter — fresh user messages get pushed, HITL continuations (`addToolOutput` / `addToolApproveResponse`) no-op because the incoming wire shares the existing assistant’s id and the runtime overlays the new tool-state advance onto that entry, and non-`submit-message` triggers (`regenerate-message` / `action`) skip persistence. It returns `true` when it mutated `stored`, so the caller knows whether to persist. If you need branching, rollback, or other custom hydrate logic, you can still write the upsert by hand — `upsertIncomingMessage` is a convenience for the common case, not the only supported shape. **Lifecycle position:** `onValidateMessages` → **`hydrateMessages`** → `onChatStart` (chat’s first message only) → `onTurnStart` → `run()` After the hook returns, the runtime overlays the wire’s tool-state advances (`output-available` / `output-error` / `approval-responded` / `output-denied`) onto matching hydrated entries by id. Everything else on the hydrated entry — text, reasoning, tool `input`, providerMetadata — stays put. This makes [tool approvals](https://trigger.dev/docs/ai-chat/frontend#tool-approvals)
 and HITL `addToolOutput` continuations work transparently: ship a slim resolution on the wire, the agent merges the new state onto your DB-backed copy.

`hydrateMessages` also fires for [action](https://trigger.dev/docs/ai-chat/actions)
 turns (`trigger: "action"`) with empty `incomingMessages`. This lets the action handler work with the latest DB state.

Registering `hydrateMessages` short-circuits the runtime’s [snapshot + replay](https://trigger.dev/docs/ai-chat/patterns/persistence-and-replay)
 reconstruction at run boot — your hook is the single source of truth for history, so the runtime skips reading or writing the snapshot entirely. No object storage traffic, no replay cost. The trade-off is that you own persistence end-to-end.

`incomingMessages` is **usually 0-or-1-length**. `submit-message` and tool-approval responses ship a single message; `regenerate-message`, continuations, and actions ship none. The exception is a [Head Start](https://trigger.dev/docs/ai-chat/fast-starts#with-hydratemessages)
 first turn, where it carries the route handler’s first-turn history. Patterns like [tool-result auditing](https://trigger.dev/docs/ai-chat/patterns/tool-result-auditing)
 work the same regardless — iterate the array rather than assuming a single element.

[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onturnstart)
...
[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onbeforeturncomplete)
...
Receives the same fields as [`TurnCompleteEvent`](https://trigger.dev/docs/ai-chat/reference#turncompleteevent)
, plus a [`writer`](https://trigger.dev/docs/ai-chat/reference#chatwriter)
.

[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onturncomplete)
...
Use `uiMessages` to overwrite the full conversation each turn (simplest). Use `newUIMessages` if you prefer to store messages individually, e.g. one database row per message.
...
[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onchatsuspend-/-onchatresume)
...
[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#exitafterpreloadidle)
...
[​](https://trigger.dev/docs/ai-chat/lifecycle-hooks#see-also)

See also
--------------------------------------------------------------------------

*   [Reference](https://trigger.dev/docs/ai-chat/reference)
     for full event-type definitions
*   [Database persistence](https://trigger.dev/docs/ai-chat/patterns/database-persistence)
     for the canonical persistence pattern
*   [Code execution sandbox](https://trigger.dev/docs/ai-chat/patterns/code-sandbox)
     for an `onChatSuspend` use case
*   [Backend](https://trigger.dev/docs/ai-chat/backend)
     for `chat.agent({ ... })` itself, prompts, stop signals, persistence overview, and runtime configuration

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/backend)
[ToolsDeclare tools on chat.agent so toModelOutput survives across turns, get them back typed in run(), and type your messages from them.\
\
Next](https://trigger.dev/docs/ai-chat/tools)
