...
Most users don’t need this. Use [`TriggerChatTransport`](https://trigger.dev/docs/ai-chat/frontend)
 for browser apps or [`AgentChat`](https://trigger.dev/docs/ai-chat/server-chat)
 for server-side code. This page is for building your own from scratch.

[​](https://trigger.dev/docs/ai-chat/client-protocol#overview)

Overview
--------------------------------------------------------------------------

`chat.agent` is built on a durable Session row — the unit of state that owns the chat’s runs across their full lifecycle. A conversation is one session; a session can host many runs over its lifetime. The protocol has three parts:
...
**One message per record.** Each `.in/append` carries at most one new `UIMessage` — the new user turn or a tool-approval response. The agent rebuilds prior history at run boot from a durable object-store snapshot plus a replay of the `session.out` tail; clients never ship full conversation history on the wire. See [How history is rebuilt](https://trigger.dev/docs/ai-chat/client-protocol#how-history-is-rebuilt)
.

[​](https://trigger.dev/docs/ai-chat/client-protocol#end-to-end-curl-recipe)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#required-fields)
...
| `triggerConfig.basePayload` | `object` | The wire payload sent to the **first run** created by this call. Same shape as [`ChatTaskWirePayload`](https://trigger.dev/docs/ai-chat/client-protocol#chattaskwirepayload)<br> in Step 3. Durable fields (`chatId`, `metadata`, `idleTimeoutInSeconds`, `sessionId`) flow through to continuation runs too; first-turn-only fields (`message`, `trigger`) are stripped on continuations — those are session-create concerns and don’t replay. See [What goes in `basePayload`](https://trigger.dev/docs/ai-chat/client-protocol#what-goes-in-basepayload)<br> below. |

### 

[​](https://trigger.dev/docs/ai-chat/client-protocol#optional-fields)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#what-goes-in-basepayload)

What goes in `basePayload`

`basePayload` is the [`ChatTaskWirePayload`](https://trigger.dev/docs/ai-chat/client-protocol#chattaskwirepayload)
 sent to the agent at run boot — the same shape used for every subsequent `.in/append` (Step 3). Two fields you must always include:
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#response)
...
| `runId` / `currentRunId` | Friendly ID of the first run. Identical on a fresh create; will diverge over the conversation (see [Continuations](https://trigger.dev/docs/ai-chat/client-protocol#continuations)<br>). |
| `publicAccessToken` | Session-scoped JWT carrying `read:sessions:{externalId}` + `write:sessions:{externalId}`. **This is the token you use for every subsequent `.in`/`.out` call.** Persist it. Lifetime is 60 minutes — see [Refreshing the token](https://trigger.dev/docs/ai-chat/client-protocol#refreshing-the-token)<br>. |
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#idempotency)
...
*   If the session is still alive: returns the existing row with `isCached: true`, `runId` unchanged, and a **fresh** 60-minute `publicAccessToken`. No duplicate run is triggered. (Idle/exited runs are different — see [Continuations](https://trigger.dev/docs/ai-chat/client-protocol#continuations)
    .)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#refreshing-the-token)

Refreshing the token

The `publicAccessToken` returned by `POST /api/v1/sessions` is valid for 60 minutes. Two ways to keep going past that:

1.  **Take refreshed tokens from the stream.** Most `turn-complete` control records on `.out` carry a `public-access-token` header with a refreshed JWT (see [`turn-complete` control record](https://trigger.dev/docs/ai-chat/client-protocol#turn-complete-control-record)
    ). The header is optional and may be absent on some turns (for example an errored turn), so replace your stored token whenever the header is present rather than expecting it every turn. For active conversations it rolls on its own.
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#step-2-subscribe-to-out)
...
If nothing arrives by the deadline, the server sends `data: [DONE]` and closes. Reconnect with `Last-Event-ID` to continue (see [Resuming a stream](https://trigger.dev/docs/ai-chat/client-protocol#resuming-a-stream)
).

### 

[​](https://trigger.dev/docs/ai-chat/client-protocol#stream-format-s2)

Stream format (S2)

The output stream uses [S2](https://s2.dev/)
 under the hood and follows the standard SSE wire format ([WHATWG spec](https://html.spec.whatwg.org/multipage/server-sent-events.html#parsing-an-event-stream)
...
The `id:` line on the wire is a comma-separated triple internal to S2 (`startSeq,endSeq,byteOffset`) — **don’t try to parse it**. Use `record.seq_num` from inside the `data` body instead (see [Resuming a stream](https://trigger.dev/docs/ai-chat/client-protocol#resuming-a-stream)
...
| `records[].body` | For data records: a JSON-encoded **string** wrapping `{ data: UIMessageChunk, id: string }`. For control records: an empty string (semantics live in `headers`). For S2 command records: opaque bytes. See [Records on session.out](https://trigger.dev/docs/ai-chat/client-protocol#records-on-session-out)<br>. |
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#records-on-session-out)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#built-in-parser-recommended-for-sdk-users)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#self-contained-parser-for-custom-transports)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#chunk-types)

Chunk types

Data records on the stream carry a `UIMessageChunk` from the [AI SDK](https://ai-sdk.dev/docs/ai-sdk-ui/ui-message-stream)
. Two Trigger.dev-specific control events ride alongside as **header-form control records** (see [Records on session.out](https://trigger.dev/docs/ai-chat/client-protocol#records-on-session-out)
). Within a single assistant turn the AI SDK chunk types you’ll typically see, in order:

| Chunk type | Shape | Notes |
| --- | --- | --- |
| `start` | `{ type: "start", messageId: string }` | First chunk of a new assistant message. **Persist `messageId`** — you’ll need it to send tool-approval responses (see [Tool approval responses](https://trigger.dev/docs/ai-chat/client-protocol#tool-approval-responses)<br>). |
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#turn-complete-control-record)
...
2.  Close the stream reader (unless you want to keep it open across turns — see [Resuming a stream](https://trigger.dev/docs/ai-chat/client-protocol#resuming-a-stream)
    ).
3.  Wait for the next user message before sending on `.in`.

### 

[​](https://trigger.dev/docs/ai-chat/client-protocol#upgrade-required-control-record)
...
The server has already swapped the run on the same session by the time this record is delivered. Subsequent records on the same SSE subscription come from the new run. When you receive this record:
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#resuming-a-stream)
...
The server resumes streaming from `seq_num = 43` onward. `Last-Event-ID` is a single non-negative integer; passing the SSE `id:` line value verbatim (e.g. `0,1,106`) silently falls back to “start from the beginning.” `SSEStreamSubscription` tracks this automatically via its `lastEventId` option.
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#step-3-send-messages-stops-and-actions)
...
`{sessionId}` accepts the same friendly-or-external forms as `.out`. The `publicAccessToken` from session-create authorizes both. The body is a JSON-serialized [`ChatInputChunk`](https://trigger.dev/docs/ai-chat/client-protocol#chatinputchunk)
 — a tagged union covering messages, stops, and actions. Send them as raw JSON strings (not wrapped in a `data` field). On success the response is `200 OK` with body `{ "ok": true }`; on failure it’s `4xx`/`5xx` with `{ "ok": false, "error": "<message>" }`. Common failures:
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#chatinputchunk)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#chattaskwirepayload)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#sending-a-message)
...
Send only the **new** user message — never the full history. The agent rebuilds prior history from a durable S3 snapshot plus a `session.out` replay at run boot. See [How history is rebuilt](https://trigger.dev/docs/ai-chat/client-protocol#how-history-is-rebuilt)
.

### 

[​](https://trigger.dev/docs/ai-chat/client-protocol#sending-a-stop)
...
Interrupts the agent’s current turn. `streamText` aborts, the agent emits a `turn-complete` control record, and the run returns to idle. An optional `message` field surfaces in the agent’s stop handler:
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#sending-an-action)
...
Actions wake the agent from suspension (same as messages) and fire the `onAction` hook — they are not turns, so `run()` and turn lifecycle hooks do not fire. If `onAction` returns a `StreamTextResult`, the response is auto-piped to the frontend (but still no `run()` or `onTurnComplete`). The `action` payload is validated against the agent’s `actionSchema`. If the agent didn’t register an `actionSchema` (or your `action` payload doesn’t match it), validation fails the same way `metadata` does — `.in/append` returns `200 OK`, but the run trace shows `chat turn N [ERROR]` and the wire emits a `turn-complete` control record with no other chunks. See [Actions](https://trigger.dev/docs/ai-chat/actions)
 for the agent-side schema setup.

### 

[​](https://trigger.dev/docs/ai-chat/client-protocol#regenerating-the-last-response)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#tool-approval-responses)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#how-history-is-rebuilt)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#path-a-%E2%80%94-hydratemessages-registered)

Path A — `hydrateMessages` registered

If the agent declares a [`hydrateMessages`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#hydratemessages)
 hook, the runtime trusts the customer to be the source of truth for history. Snapshot read and replay are **skipped entirely** at boot. The hook fires per turn — `incomingMessages` is 0-or-1-length consistently (since each record carries at most one new message) — and returns the canonical chain from the customer’s database.

### 

[​](https://trigger.dev/docs/ai-chat/client-protocol#path-b-%E2%80%94-snapshot-+-replay-default)
...
Write a fresh snapshot after every turn
...
Object-store configuration is the same as the rest of Trigger.dev — set `OBJECT_STORE_*` env vars. With no object store configured and no `hydrateMessages` hook, conversations don’t survive run boundaries; the runtime logs a warning at registration time. For a deeper walkthrough of the snapshot model, including OOM-retry interaction and crash semantics, see [Persistence and replay](https://trigger.dev/docs/ai-chat/patterns/persistence-and-replay)
.

[​](https://trigger.dev/docs/ai-chat/client-protocol#head-start-protocol-caveat)

Head-start protocol caveat
--------------------------------------------------------------------------------------------------------------

The [`chat.headStart`](https://trigger.dev/docs/ai-chat/fast-starts#head-start)
 flow runs the first turn’s LLM call inside the customer’s own HTTP route handler, then hands the durable stream off to the agent for tool execution and step 2+. On that first-ever turn no snapshot exists yet — the agent boots empty. To bridge that gap, the head-start route handler ships **full UIMessage history** through the dedicated `headStartMessages` field with `trigger: "handover-prepare"`. This is the **only** path where a wire-shipped UIMessage\[\] still seeds the agent’s accumulator:
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#pending-and-steering-messages)

Pending and steering messages
--------------------------------------------------------------------------------------------------------------------

You can send messages while the agent is still streaming a response. These are **pending messages** — the agent receives them mid-turn and can inject them between tool-call steps. The wire format is identical to a normal `kind: "message"` send — same `.in` channel, single `message` field. The difference is timing. What happens depends on the agent’s `pendingMessages` configuration:
...
See [Pending Messages](https://trigger.dev/docs/ai-chat/pending-messages)
 for how to configure the agent side.
...
This is how [version upgrades](https://trigger.dev/docs/ai-chat/patterns/version-upgrades)
 work transparently — the agent calls `chat.requestUpgrade()`, the run exits, and the client’s next message triggers a continuation on the new version. Same session, new run, same snapshot.

[​](https://trigger.dev/docs/ai-chat/client-protocol#closing-the-conversation)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#session-state)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#authentication)
...
[​](https://trigger.dev/docs/ai-chat/client-protocol#faq)

FAQ
----------------------------------------------------------------

Show After sending \`kind: stop\`, can I immediately send the next message?
...
Show What's the format of the optional \`X-Part-Id\` header?
...
Show What happens on rate-limit (429)?
...
Show How do I tell from the \`.out\` stream that a run has ended (vs idled between turns)?
...
Show Does the \`seq\_num\` reset across continuations or runs?
...
Show What's the maximum size of a single \`.in/append\` body?

The HTTP body is capped at 1 MiB as a DoS guard. The actual ceiling is at the storage layer: each `.in/append` becomes a single S2 record, metered as `8 + body_bytes_after_JSON_wrap`, capped at 1 MiB. So the practical limit on the raw HTTP body sits around ~1023 KiB for content with low JSON-escape overhead (ASCII, base64) and ~512 KiB for content that escapes heavily (all quotes / backslashes). A typical `kind: "message"` is a few KiB. If you’re brushing the cap you’re either shipping a single tool output that’s itself oversized — see [Large payloads](https://trigger.dev/docs/ai-chat/patterns/large-payloads)
 — or you’re shipping more than one message per record, which the protocol forbids. The 413 response carries CORS headers so browser fetches can read the status. The headStart path (`trigger: "handover-prepare"`) sends through the customer’s own HTTP route handler, not `.in/append`, so the cap doesn’t apply there.

[​](https://trigger.dev/docs/ai-chat/client-protocol#see-also)

See also
--------------------------------------------------------------------------

*   [`TriggerChatTransport`](https://trigger.dev/docs/ai-chat/frontend)
     — Built-in browser transport (implements this protocol)
*   [`AgentChat`](https://trigger.dev/docs/ai-chat/server-chat)
     — Built-in server-side client
*   [Persistence and replay](https://trigger.dev/docs/ai-chat/patterns/persistence-and-replay)
     — How the snapshot + replay model works end-to-end
*   [Lifecycle hooks](https://trigger.dev/docs/ai-chat/lifecycle-hooks)
     — What the agent does on each event
*   [Version upgrades](https://trigger.dev/docs/ai-chat/patterns/version-upgrades)
     — How `chat.requestUpgrade()` uses continuations

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/reference)
[TestingDrive a chat.agent through real turns in unit tests — no network, no task runtime, no mocking the SDK.\
\
Next](https://trigger.dev/docs/ai-chat/testing)
