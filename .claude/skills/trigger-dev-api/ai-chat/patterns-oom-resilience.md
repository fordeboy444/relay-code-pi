...
`chat.agent` does not expose generic `retry` options. OOM recovery is the only retry path because retrying an LLM-driven loop on non-OOM errors tends to be expensive and side-effecting. Drop down to a [raw `task()` with chat primitives](https://trigger.dev/docs/ai-chat/custom-agents)
 if you need richer retry semantics.

[​](https://trigger.dev/docs/ai-chat/patterns/oom-resilience#how-recovery-works)
...
Result: turns 1..N-1 are not re-processed, turn N runs on the larger machine, and the conversation continues. The scan on `session.out` is streaming and bounded in memory: each chunk is inspected and discarded one at a time, so a long-running chat doesn’t bloat the retry-boot worker. Bandwidth scales linearly with `session.out` size, but only on the OOM-retry path — a rare event.

[​](https://trigger.dev/docs/ai-chat/patterns/oom-resilience#with-hydratemessages)

With `hydrateMessages`
------------------------------------------------------------------------------------------------------------

If your agent uses [`hydrateMessages`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#hydratemessages)
 to load the durable conversation history per turn, the OOM’d turn re-runs against the full prior accumulator: the model sees `[u1, a1, u2, a2, ..., u_N]` and responds in context. This is the recommended pattern for production chats.

[​](https://trigger.dev/docs/ai-chat/patterns/oom-resilience#without-hydratemessages)
...
[​](https://trigger.dev/docs/ai-chat/patterns/oom-resilience#tool-execute-idempotency)
...
[​](https://trigger.dev/docs/ai-chat/patterns/oom-resilience#limitations)
...
*   **Single fallback tier.** Only one `oomMachine`. There’s no “tiered retry” (small → medium → large). If you need that, drop down to a [raw `task()` with chat primitives](https://trigger.dev/docs/ai-chat/custom-agents)
     and configure `retry` directly.
...
[​](https://trigger.dev/docs/ai-chat/patterns/oom-resilience#see-also)

See also
----------------------------------------------------------------------------------

*   [Recovery boot](https://trigger.dev/docs/ai-chat/patterns/recovery-boot)
     — the underlying hook + smart default that gives OOM recovery its full-context behavior
*   [Lifecycle hooks](https://trigger.dev/docs/ai-chat/lifecycle-hooks)
     — `onChatResume` fires on every retry attempt with `phase: "preload"` or `"turn"`
*   [Database persistence](https://trigger.dev/docs/ai-chat/patterns/database-persistence)
     — the `hydrateMessages` pattern for branching, ACL, and DB-as-source-of-truth scenarios

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/patterns/skills)
[Recovery bootRecover from cancel-mid-stream, crashes, and OOM kills with full conversational context. The smart default Just Works; the onRecoveryBoot hook is the override path for advanced policies.\
\
Next](https://trigger.dev/docs/ai-chat/patterns/recovery-boot)
