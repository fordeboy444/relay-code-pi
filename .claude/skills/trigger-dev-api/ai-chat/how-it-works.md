...
This page explains how `chat.agent` is put together, what each piece does on a single turn, and how a chat survives across turns. It is not an API tour — for that, see [Backend](https://trigger.dev/docs/ai-chat/backend)
, [Frontend](https://trigger.dev/docs/ai-chat/frontend)
, and the [Reference](https://trigger.dev/docs/ai-chat/reference)
. For the byte-level wire format, see [Client Protocol](https://trigger.dev/docs/ai-chat/client-protocol)
.
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#the-primary-noun-the-chat-session)
...
Both channels are S2 ([s2.dev](https://s2.dev/)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#the-lifecycle-states)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#cold-start)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#suspended)

Suspended

The task fires `onChatSuspend`, then the engine **checkpoints** the run’s whole process state and frees the compute. The session is still live (the row exists, the `.out` stream is still readable, the chat ID still works), but no machine is dedicated to it. This is the same Checkpoint-Resume System that powers every Trigger.dev task — covered in detail at [How it works → Checkpoint-Resume](https://trigger.dev/docs/how-it-works#the-checkpoint-resume-system)
. Moves to **Resuming** when the next message lands in `.in`.

### 

[​](https://trigger.dev/docs/ai-chat/how-it-works#resuming)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#one-turn-end-to-end)
...
Browser: useChat calls transport.sendMessages
...
Browser: transport posts to /append
...
Server: route ensures a run exists
...
Server: route appends the record to S2 .in
...
Browser: transport opens an SSE subscription to .out
...
Task: enters the turn loop, reads the message from .in
...
Task: runs your run() function, streams chunks to .out
...
Task: writes the turn-complete control record
...
Task: trims .out back to the previous turn-complete

Immediately after writing the new turn-complete marker, the agent issues an S2 trim command targeting the _previous_ turn-complete’s sequence number. This bounds the stream’s storage to roughly one turn of chunks plus the latest control record.

11

[](https://trigger.dev/docs/ai-chat/how-it-works#)

Task: fires onTurnComplete, writes snapshot to S3
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#three-layers-of-persistence)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#which-layer-covers-which-failure-mode)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#warm-vs-cold-same-chat-three-different-timings)
...
[​](https://trigger.dev/docs/ai-chat/how-it-works#lifecycle-hooks-where-you-plug-in)
...
See [Lifecycle hooks](https://trigger.dev/docs/ai-chat/lifecycle-hooks)
 for the full signatures and firing order.

[​](https://trigger.dev/docs/ai-chat/how-it-works#when-chat-agent-is-the-right-primitive)
...
*   Workflows where you control both ends and want a custom protocol. Use a [raw `task()` with chat primitives](https://trigger.dev/docs/ai-chat/custom-agents)
     directly without the `chat.agent` wrapper.
*   High-fanout broadcasting (one source, many subscribers). Use Trigger.dev realtime streams against a regular task instead.

[​](https://trigger.dev/docs/ai-chat/how-it-works#putting-it-together)

Putting it together
---------------------------------------------------------------------------------------------

[​](https://trigger.dev/docs/ai-chat/how-it-works#where-to-go-next)

Where to go next
---------------------------------------------------------------------------------------

*   [Quick start](https://trigger.dev/docs/ai-chat/quick-start)
     — get a chat running in a few minutes.
*   [Backend](https://trigger.dev/docs/ai-chat/backend)
     — the `chat.agent()` API in detail.
*   [Lifecycle hooks](https://trigger.dev/docs/ai-chat/lifecycle-hooks)
     — every hook, what fires when.
*   [Persistence and replay](https://trigger.dev/docs/ai-chat/patterns/persistence-and-replay)
     — deeper on the snapshot model.
*   [Client protocol](https://trigger.dev/docs/ai-chat/client-protocol)
     — wire format if you’re writing a custom transport.

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/custom-agents)
[PromptsDefine prompt templates as code, version them on deploy, and override from the dashboard without redeploying.\
\
Next](https://trigger.dev/docs/ai/prompts)
