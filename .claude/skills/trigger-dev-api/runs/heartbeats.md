...
If you see `TASK_RUN_STALLED_EXECUTING`, see [Task run stalled executing](https://trigger.dev/docs/troubleshooting#task-run-stalled-executing)
 in the troubleshooting guide.

[​](https://trigger.dev/docs/runs/heartbeats#sending-progress-to-trigger-dev)

Sending progress to Trigger.dev
----------------------------------------------------------------------------------------------------------------

To stream progress or status updates to the dashboard and your app, use [run metadata](https://trigger.dev/docs/runs/metadata)
. Call `metadata.set()` (or `metadata.append()`) as the task runs. The dashboard and [Realtime](https://trigger.dev/docs/realtime)
 (including `runs.subscribeToRun` and the React hooks) receive those updates as they happen. See [Progress monitoring](https://trigger.dev/docs/realtime/backend/subscribe#progress-monitoring)
 for a full example.

[​](https://trigger.dev/docs/runs/heartbeats#sending-updates-to-your-own-system)
...
[Previous](https://trigger.dev/docs/runs/max-duration)
[TagsTags allow you to easily filter runs in the dashboard and when using the SDK.\
\
Next](https://trigger.dev/docs/tags)
