...
Every session has two durable streams: `.in` carries records from your clients to the task, `.out` carries records from the task back to your clients. The [`sessions` SDK](https://trigger.dev/docs/ai-chat/sessions)
 wraps these as `session.in.*` and `session.out.*`. This page documents the underlying HTTP endpoints for callers that aren’t using the TypeScript SDK. All channel endpoints live under `/realtime/v1/sessions/{session}/{io}`, where:

*   `{session}` is the session’s friendly ID (`session_…`) or your `externalId`. One token authorizes both forms.
*   `{io}` is either `in` or `out`.

Authorize requests with a secret key or a [session public token](https://trigger.dev/docs/management/authentication#session-scopes)
. The token’s scopes decide what you can do — see [Authorization](https://trigger.dev/docs/management/sessions/channels#authorization)
 below.

[​](https://trigger.dev/docs/management/sessions/channels#append-a-record)
...
[​](https://trigger.dev/docs/management/sessions/channels#read-a-channel-over-sse)
...
[​](https://trigger.dev/docs/management/sessions/channels#control-records)
...
[​](https://trigger.dev/docs/management/sessions/channels#drain-records-non-streaming)
...
[​](https://trigger.dev/docs/management/sessions/channels#authorization)
...
Reads work in both directions for a `read:sessions` token. Writes split by direction: a `write:sessions` token can append to `.in`, but `.out` is reserved for the task and requires a secret key. See [session scopes](https://trigger.dev/docs/management/authentication#session-scopes)
 for how to mint a token.

[​](https://trigger.dev/docs/management/sessions/channels#using-the-sdk-instead)

Using the SDK instead
---------------------------------------------------------------------------------------------------------

If you’re writing TypeScript, the [`sessions` SDK](https://trigger.dev/docs/ai-chat/sessions)
 is the ergonomic path. `sessions.open(idOrExternalId)` returns a `SessionHandle` whose `session.in` and `session.out` channels call these endpoints for you, with auto-retry, `Last-Event-ID` resume, and control-record routing built in:
...
See [`session.in`](https://trigger.dev/docs/ai-chat/sessions#session-in-%E2%80%94-clients-%E2%86%92-task)
 and [`session.out`](https://trigger.dev/docs/ai-chat/sessions#session-out-%E2%80%94-task-%E2%86%92-clients)
 for the full handle API.

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/management/sessions/close)
[Execute a queryExecute a TRQL (Trigger.dev Query Language) query against your run data. TRQL is a SQL-style query language that allows you to analyze runs, calculate metrics, and export data.\
\
Next](https://trigger.dev/docs/management/query/execute)
