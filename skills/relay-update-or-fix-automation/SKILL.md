...
If `/skill:relay-update-or-fix-automation` is invoked without supplying either, ask (AskUserQuestion):
...
|---|---|
| No spec at `docs/specs/<slug>.md` | Decline: "No spec found for `<slug>`. Run `/skill:relay-research-automation <slug>` first — that skill writes the spec; this one only updates existing specs." |
| A brand-new automation | Decline: "That sounds like a new automation. Run `/skill:relay-research-automation` to design it." |
...
| Operational / procedural request (deploy, dev server up) | Decline with a pointer to the matching runbook inside `/skill:relay-execute-or-resume-automation` (the deploy-order tools, `relay_dev_worker`). |
| A follow-up plan already exists covering this exact change | Decline: "There's already a follow-up plan covering this — run `/skill:relay-execute-or-resume-automation <slug>` to resume it, or pick a different change." |
...
**Decline gate:** if a follow-up plan **already exists** whose `**Goal:**` matches the requested change, decline with the already-covered message above and point the user at `/skill:relay-execute-or-resume-automation <slug>`.
...
### 4. Invoke `/skill:relay-plan-automation` with the follow-up preamble.

Prepend this preamble to whatever `/skill:relay-plan-automation` produces. The preamble is required so the plan skill writes the right file with the right framing:
...
The plan skill today produces a one-liner — that is wrong for a follow-up. **After `/skill:relay-plan-automation` writes the file, replace the plan's `## Spec` block with the verbatim follow-up block above**, before handing off to execute. Do not paraphrase it — it is a load-bearing contract.
...
- **Never write the follow-up spec as a new file under `docs/specs/`.** The parent spec stays the only spec; follow-ups go into `docs/plans/`. If the user wants a separate doc per change, redirect them to `/skill:relay-research-automation`.
...
- **Never let the follow-up plan skip the "Spec — fold this change in" terminal task.** Edit the plan to add it before approval if `/skill:relay-plan-automation` produced a plan without it.
