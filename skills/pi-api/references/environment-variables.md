...
- **URL:** https://pi.dev/docs/latest/environment-variables
- **Command:** `Environment Variables`
- **Summary:** Pi uses environment variables in three ways: - Variables such as `PI_OFFLINE` configure the Pi process. - Pi sets process markers so child processes can identify Pi as the launching agent. - Commands run by the LLM-callable bash tool receive `PI_*` variables describing the current session.
