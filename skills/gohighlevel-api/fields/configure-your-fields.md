...
- **URL:** https://marketplace.gohighlevel.com/docs/oauth/external-auth/ConfigureYourFields
- **Summary:** Configure Your Fields lets you collect custom inputs from the user during installation. These inputs become available to every request your app makes through External Authentication, so you can tailor URLs, headers, parameters, and request bodies to each individual user.

[Skip to main content](https://marketplace.gohighlevel.com/docs/oauth/external-auth/ConfigureYourFields#__docusaurus_skipToContent_fallback)
...
This section is available for **both** [API Key / Basic Auth](https://marketplace.gohighlevel.com/docs/oauth/external-auth/BasicAuth)
 and [OAuth 2.0](https://marketplace.gohighlevel.com/docs/oauth/external-auth/OAuth2)
.
...
> Using [Code Mode](https://marketplace.gohighlevel.com/docs/oauth/external-auth/CodeMode)
> ? The same values are available there too - access them through `bundle.inputData.<key>` instead of the `{{userData.<key>}}` template syntax.
...
*   [Why custom fields exist: dynamic, per-user endpoints](https://marketplace.gohighlevel.com/docs/oauth/external-auth/ConfigureYourFields#why-custom-fields-exist-dynamic-per-user-endpoints)
    
*   [Adding a field](https://marketplace.gohighlevel.com/docs/oauth/external-auth/ConfigureYourFields#adding-a-field)
    
*   [Using field values: the `userData` object](https://marketplace.gohighlevel.com/docs/oauth/external-auth/ConfigureYourFields#using-field-values-the-userdata-object)
    
*   [Where the values are sent](https://marketplace.gohighlevel.com/docs/oauth/external-auth/ConfigureYourFields#where-the-values-are-sent)
