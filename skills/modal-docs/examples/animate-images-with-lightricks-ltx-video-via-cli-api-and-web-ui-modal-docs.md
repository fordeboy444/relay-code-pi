# Animate images with Lightricks LTX-Video via CLI, API, and web UI | Modal Docs

- **URL:** https://modal.com/docs/examples/image_to_video
- **Summary:** This example shows how to run LTX-Video on Modal to generate videos from your local command line, via an API, and in a web UI.
...
This example shows how to run [LTX-Video](https://huggingface.co/Lightricks/LTX-Video)
 on Modal to generate videos from your local command line, via an API, and in a web UI.
...
Below, we start from a lightweight base Linux image and then install our system and Python dependencies, like Hugging Face’s `diffusers` library and `torch`.
...
We can fix this — without any modifications to Hugging Face’s model loading code! — by pointing the Hugging Face cache at a [Modal Volume](https://modal.com/docs/guide/volumes)
...
We wrap the inference logic in a Modal [Cls](https://modal.com/docs/guide/lifecycle-functions)
 that ensures models are loaded and then moved to the GPU once when a new instance starts, rather than every time we run it.

The `run` function just wraps a `diffusers` pipeline. It saves the generated video to a Modal Volume, and returns the filename.

We also include a `web` wrapper that makes it possible to trigger inference via an API call. For details, see the `/docs` route of the URL ending in `inference-web.modal.run` that appears when you deploy the app.
...
We add a [local entrypoint](https://modal.com/docs/reference/modal.App#local_entrypoint)
 that calls the `Inference.run` method to run inference from the command line. The function’s parameters are automatically turned into a CLI.
...
[Animate images with Lightricks LTX-Video via CLI, API, and web UI](https://modal.com/docs/examples/image_to_video#animate-images-with-lightricks-ltx-video-via-cli-api-and-web-ui)
[Basic setup](https://modal.com/docs/examples/image_to_video#basic-setup)
[Configuring dependencies](https://modal.com/docs/examples/image_to_video#configuring-dependencies)
[Storing model weights on Modal](https://modal.com/docs/examples/image_to_video#storing-model-weights-on-modal)
[Storing model outputs on Modal](https://modal.com/docs/examples/image_to_video#storing-model-outputs-on-modal)
[Implementing LTX-Video inference on Modal](https://modal.com/docs/examples/image_to_video#implementing-ltx-video-inference-on-modal)
[Generating videos from the command line](https://modal.com/docs/examples/image_to_video#generating-videos-from-the-command-line)
[Generating videos via an API](https://modal.com/docs/examples/image_to_video#generating-videos-via-an-api)
[Generating videos in a web UI](https://modal.com/docs/examples/image_to_video#generating-videos-in-a-web-ui)
