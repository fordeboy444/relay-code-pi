...
In this example, we demonstrate how to use the the [ColQwen2](https://huggingface.co/vidore/colqwen2-v0.1)
 model to build a simple “Chat with PDF” retrieval-augmented generation (RAG) app. The ColQwen2 model is based on [ColPali](https://huggingface.co/blog/manu/colpali)
 but uses the [Qwen2-VL-2B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-2B-Instruct)
 vision-language model. ColPali is in turn based on the late-interaction embedding approach pioneered in [ColBERT](https://dl.acm.org/doi/pdf/10.1145/3397271.3401075)
.

Vision-language models with high-quality embeddings obviate the need for complex pre-processing pipelines. See [this blog post from Jo Bergum of Vespa](https://blog.vespa.ai/announcing-colbert-embedder-in-vespa/)
 for more.
...
In Modal, we define [container images](https://modal.com/docs/guide/custom-container)
 that run our serverless workloads. We install the packages required for our application in those images.
...
Modal Functions are stateless in and of themselves. They don’t retain information from input to input. That’s what enables Modal Functions to automatically scale up and down [based on the number of incoming requests](https://modal.com/docs/guide/cold-start)
.
...
Modal Dicts behave similarly to Python dictionaries, but they are backed by remote storage and accessible to all of your Modal Functions. They can contain any Python object that can be serialized using [`cloudpickle`](https://github.com/cloudpipe/cloudpickle)
.
...
Running this function will download the model weights to the cache volume. Otherwise, the model weights will be downloaded on the first query. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights)
.
...
To deploy an autoscaling “Chat with PDF” vision-language model service on Modal, we just need to wrap our Python logic in a [Modal App](https://modal.com/docs/guide/apps)
:

It uses [Modal `@app.cls`](https://modal.com/docs/guide/lifecycle-functions)
 decorators to organize the “lifecycle” of the app: loading the model on container start (`@modal.enter`) and running inference on request (`@modal.method`).
...
With the [Gradio](https://gradio.app/)
 library, we can create a simple web interface around our class in Python, then use Modal to host it for anyone to try out.
...
[Chat with PDF: RAG with ColQwen2](https://modal.com/docs/examples/chat_with_pdf_vision#chat-with-pdf-rag-with-colqwen2)
[Setup](https://modal.com/docs/examples/chat_with_pdf_vision#setup)
[Setting up dependencies](https://modal.com/docs/examples/chat_with_pdf_vision#setting-up-dependencies)
[Specifying the ColQwen2 model](https://modal.com/docs/examples/chat_with_pdf_vision#specifying-the-colqwen2-model)
[Managing state with Modal Volumes and Dicts](https://modal.com/docs/examples/chat_with_pdf_vision#managing-state-with-modal-volumes-and-dicts)
[Managing chat sessions with Modal Dicts](https://modal.com/docs/examples/chat_with_pdf_vision#managing-chat-sessions-with-modal-dicts)
[Storing PDFs on a Modal Volume](https://modal.com/docs/examples/chat_with_pdf_vision#storing-pdfs-on-a-modal-volume)
[Caching the model weights](https://modal.com/docs/examples/chat_with_pdf_vision#caching-the-model-weights)
[Defining a Chat with PDF service](https://modal.com/docs/examples/chat_with_pdf_vision#defining-a-chat-with-pdf-service)
[Loading PDFs as images](https://modal.com/docs/examples/chat_with_pdf_vision#loading-pdfs-as-images)
[Chatting with a PDF from the terminal](https://modal.com/docs/examples/chat_with_pdf_vision#chatting-with-a-pdf-from-the-terminal)
[A hosted Gradio interface](https://modal.com/docs/examples/chat_with_pdf_vision#a-hosted-gradio-interface)
[Addenda](https://modal.com/docs/examples/chat_with_pdf_vision#addenda)
