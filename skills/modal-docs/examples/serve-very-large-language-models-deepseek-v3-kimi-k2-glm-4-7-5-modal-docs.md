# Serve very large language models (DeepSeek V3, Kimi-K2, GLM 4.7/5) | Modal Docs

- **URL:** https://modal.com/docs/examples/very_large_models
- **Summary:** This example demonstrates the basic patterns for serving language models on Modal whose weights consume hundreds of gigabytes of storage.

[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/llm-serving/very_large_models.py)
...
For more tips on how to serve specific types of LLM inference at high performance, see [this guide](https://modal.com/docs/guide/high-performance-llm-inference)
. For a gentler introduction to LLM serving, see [this example](https://modal.com/docs/examples/llm_inference)
.
...
After loading them we can cache these weights in a Modal [Volume](https://modal.com/docs/guide/volumes)
 so that they are loaded about 10x faster — about one to three gigabytes per second.
...
Most importantly, we need to decide what hardware to run on. [H200 and B200 GPUs](https://modal.com/blog/introducing-b200-h200)
 have over 100 GB of [GPU RAM](https://modal.com/gpu-glossary/device-hardware/gpu-ram)
 — 141 GB and 180 GB, respectively. The model’s weights will be stored in this memory, and they consume several hundred gigabytes of space, so we will generally want several of these accelerators. We also need space for the model’s KV cache of activations on input sequences.
...
We’ll use a Modal Server to serve our model. This reduces client latencies and provides for regionalized deployment. You can read more about it in [this example](https://modal.com/docs/examples/sglang_low_latency)
. To configure it, we need to pass in region information for the GPU workers and for the load-balancing proxy.
...
The Modal Server allows us to control [container lifecycle](https://modal.com/docs/guide/lifecycle-functions)
. In particular, it lets us define work that a replica should do before and after it handles requests in methods decorated with `modal.enter` and `modal.exit`, respectively.
...
[Serve very large language models (DeepSeek V3, Kimi-K2, GLM 4.7/5)](https://modal.com/docs/examples/very_large_models#serve-very-large-language-models-deepseek-v3-kimi-k2-glm-475)
[Set up the container image](https://modal.com/docs/examples/very_large_models#set-up-the-container-image)
[Load model weights](https://modal.com/docs/examples/very_large_models#load-model-weights)
[Configure the inference engine](https://modal.com/docs/examples/very_large_models#configure-the-inference-engine)
[Configure infrastructure](https://modal.com/docs/examples/very_large_models#configure-infrastructure)
[Define the server](https://modal.com/docs/examples/very_large_models#define-the-server)
[Test the server](https://modal.com/docs/examples/very_large_models#test-the-server)
[Deploy the server](https://modal.com/docs/examples/very_large_models#deploy-the-server)
[Addenda](https://modal.com/docs/examples/very_large_models#addenda)
> Common reference appendix (shared error/status catalog): see [../_shared-appendix.md](../_shared-appendix.md).
    modal run 06_gpu_and_ml/llm-serving/very_large_models.py
