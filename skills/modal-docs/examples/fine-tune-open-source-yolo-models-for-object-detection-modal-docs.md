# Fine-tune open source YOLO models for object detection | Modal Docs

- **URL:** https://modal.com/docs/examples/finetune_yolo
- **Summary:** Example by @Erik-Dunteman and @AnirudhRahul.
...
Example by [@Erik-Dunteman](https://github.com/erik-dunteman)
 and [@AnirudhRahul](https://github.com/AnirudhRahul/)
.

The popular “You Only Look Once” (YOLO) model line provides high-quality object detection in an economical package. In this example, we use the [YOLOv10](https://docs.ultralytics.com/models/yolov10/)
 model, released on May 23, 2024.

We will:

*   Download two custom datasets from the [Roboflow](https://roboflow.com/)
     computer vision platform: a dataset of cats and a dataset of dogs
...
Modal runs your code in the cloud inside containers. So to use it, we have to define the dependencies of our code as part of the container’s [image](https://modal.com/docs/guide/custom-container)
.
...
We also create a persistent [Volume](https://modal.com/docs/guide/volumes)
 for storing datasets, trained weights, and inference outputs. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights)
.
...
*   Set up a Modal [Secret](https://modal.com/docs/guide/secrets)
     called `roboflow-api-key` in the Modal UI [here](https://modal.com/secrets)
...
We use the `@enter` feature of [`modal.Cls`](https://modal.com/docs/guide/lifecycle-functions)
 to load the model only once on container start and reuse it for future inferences. We use a generator to stream images to the model.
...
[Fine-tune open source YOLO models for object detection](https://modal.com/docs/examples/finetune_yolo#fine-tune-open-source-yolo-models-for-object-detection)
[Set up the environment](https://modal.com/docs/examples/finetune_yolo#set-up-the-environment)
[Download a dataset](https://modal.com/docs/examples/finetune_yolo#download-a-dataset)
[Train a model](https://modal.com/docs/examples/finetune_yolo#train-a-model)
[Run inference on single inputs and on streams](https://modal.com/docs/examples/finetune_yolo#run-inference-on-single-inputs-and-on-streams)
[Running the example](https://modal.com/docs/examples/finetune_yolo#running-the-example)
[Addenda](https://modal.com/docs/examples/finetune_yolo#addenda)
