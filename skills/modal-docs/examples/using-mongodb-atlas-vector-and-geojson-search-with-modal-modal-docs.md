...
- **URL:** https://modal.com/docs/examples/mongodb-search
- **Summary:** This example repo demonstrates how to use Modal and MongoDB together to build a full-stack application.
...
This [example repo](https://github.com/modal-labs/search-california)
 demonstrates how to use Modal and MongoDB together to build a full-stack application.
...
We use the [Clay foundation model](https://clay-foundation.github.io/model/index.html)
 for embeddings and we source the images from the European Space Agency’s [Sentinel satellites](https://www.esa.int/Applications/Observing_the_Earth/Copernicus/The_Sentinel_missions)
.
...
1.  Every few days, the European Space Agency’s [Sentinel Satellites](https://www.esa.int/Applications/Observing_the_Earth/Copernicus/The_Sentinel_missions)
     complete a full pass over the entire Earth, including California. The images are made available via a [public STAC API](https://element84.com/geospatial/introducing-earth-search-v1-new-datasets-now-available/)
    .
...
4.  We host a database client on Modal that allows the application’s developers to manipulate the data. This client is also used by two Web Functions for vector and geospatial search queries powered by Atlas Search.
...
Follow the instructions [here](https://modal.com/docs/guide#getting-started)
 to set up your Modal account. The $30/month of compute included in Modal’s free tier is more than enough to deploy and host this example.

You’ll also need an account on MongoDB Atlas. You can find instructions [here](https://www.mongodb.com/docs/atlas/getting-started/)
. We prefer the UI, rather than the CLI, for setup. The free tier is more than sufficient to run this example.

You’ll want to create a database called `modal-examples`. Make sure it’s accessible from [all IP addresses](https://stackoverflow.com/questions/66035947/allow-access-from-anywhere-mongodb-atlas)
. In the process, you will create a database user with a password. Navigate to the Modal Secrets dashboard [here](https://modal.com/secrets)
 and add this information, as well as the connection string for your database, to a Modal Secret based on the MongoDB template available in the dashboard.
...
By default, it’s the state of California as defined by the GeoJSON in this repository’s `data` folder (originally retrieved from [the `geojsonio` GitHub repository](https://github.com/ropensci/geojsonio/blob/7e4cc683ed3d6eec38a8cae5ce03fa6d82acafc7/inst/examples/california.geojson)
). You can pass a different GeoJSON file to the `add_aoi` action with the `--target` flag.
...
Those webhooks come with interactive OpenAPI docs, which you can access by navigating to the `/docs` route of the deployment’s URL. You should see that URL in the terminal output. You can also find the URL in the app’s [Modal dashboard](https://modal.com/apps)
.

For our deployment, the URL for the interactive docs for the geographic search endpoint is [`https://modal-labs-examples--clay-mongo-client-geo-search.modal.run/docs`](https://modal-labs-examples--clay-mongo-client-geo-search.modal.run/docs)
.
...
[Using MongoDB Atlas Vector and GeoJSON Search with Modal](https://modal.com/docs/examples/mongodb-search#using-mongodb-atlas-vector-and-geojson-search-with-modal)
[Overview](https://modal.com/docs/examples/mongodb-search#overview)
[Deploying the Backend](https://modal.com/docs/examples/mongodb-search#deploying-the-backend)
[Setup: Modal and MongoDB Atlas](https://modal.com/docs/examples/mongodb-search#setup-modal-and-mongodb-atlas)
[MongoDB Client (database.py)](https://modal.com/docs/examples/mongodb-search#mongodb-client-databasepy)
[Backfill and Updates (extract.py)](https://modal.com/docs/examples/mongodb-search#backfill-and-updates-extractpy)
[Clay Embeddings Service (embeddings.py)](https://modal.com/docs/examples/mongodb-search#clay-embeddings-service-embeddingspy)
[Putting It All Together](https://modal.com/docs/examples/mongodb-search#putting-it-all-together)
[Deploying the Frontend](https://modal.com/docs/examples/mongodb-search#deploying-the-frontend)
[Alpine App (app.js)](https://modal.com/docs/examples/mongodb-search#alpine-app-appjs)
[FastAPI Server (serve.py)](https://modal.com/docs/examples/mongodb-search#fastapi-server-servepy)
