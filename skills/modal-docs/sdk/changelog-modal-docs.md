# Changelog | Modal Docs

- **URL:** https://modal.com/docs/sdk/py/changelog
- **Summary:** This changelog documents user-facing updates (features, enhancements, fixes, and deprecations) to the modal client library.
...
This release includes several major new features, including the debut of a new serverless compute primitive for low-latency HTTP applications ([`@app.server()`](https://modal.com/docs/sdk/py/latest/modal.App#server)
) and a new CLI for deploying production-ready LLM inference endpoints with minimal configuration ([`modal endpoint`](https://modal.com/docs/cli/latest/endpoint)
).

*   We’re introducing a new [`@app.server()`](https://modal.com/docs/sdk/py/latest/modal.App#server)
     decorator (and [`modal.Server`](https://modal.com/docs/sdk/py/latest/modal.Server)
     object), representing a serverless compute primitive that shares many features with Modal Functions while being optimized for serving HTTP-based applications with ultra-low latency. Read the [guide](https://modal.com/docs/guide/servers)
     for more information.
*   We’re also introducing the [`modal endpoint`](https://modal.com/docs/cli/latest/endpoint)
     CLI, providing programmatic access to our new [Endpoints](https://modal.com/docs/guide/endpoints)
     product. Endpoints let you deploy production-ready LLM inference servers with minimal configuration.
*   We’ve added several new SDK features for workspace configuration and observability:
    *   We’ve added new billing API methods on the [`modal.Workspace`](https://modal.com/docs/sdk/py/latest/modal.Workspace)
         and [`modal.Environment`](https://modal.com/docs/sdk/py/latest/modal.Environment)
         objects ([`workspace.billing.report()`](https://modal.com/docs/sdk/py/latest/modal.Workspace#billingreport)
         and [`environment.billing.report()`](https://modal.com/docs/sdk/py/latest/modal.Environment#billingreport)
        , respectively). The new APIs include a resource-level cost breakdown (CPU, memory, and specific GPU types). We’ve also added a [`modal environment billing`](https://modal.com/docs/cli/latest/environment#modal-environment-billing)
         CLI for generating environment-scoped billing reports. The new workspace-level API replaces the existing [`modal.billing.workspace_billing_report`](https://modal.com/docs/sdk/py/latest/modal.billing#modalbillingworkspace_billing_report)
         function.
    *   We’ve added support for creating and managing [proxy tokens](https://modal.com/docs/guide/webhook-proxy-auth)
         via the [`modal.Workspace`](https://modal.com/docs/sdk/py/latest/modal.Workspace)
         object ([`workspace.proxy_tokens.create()`](https://modal.com/docs/sdk/py/latest/modal.Workspace#proxy_tokenscreate)
        , [`workspace.proxy_tokens.list()`](https://modal.com/docs/sdk/py/latest/modal.Workspace#proxy_tokenslist)
        , etc.) and a new [`modal workspace proxy-tokens`](https://modal.com/docs/cli/latest/workspace#modal-workspace-proxy-tokens)
         CLI. Modal Servers and Endpoints are authenticated via proxy tokens by default.
    *   We’ve added a new [`modal workspace members`](https://modal.com/docs/cli/latest/workspace#modal-workspace-members)
         CLI for querying information about workspace membership.
*   The new [`modal curl`](https://modal.com/docs/cli/latest/curl)
     CLI command allows you to make requests to authenticated endpoints without explicitly passing proxy token headers. This is an experimental feature and may change in the future.
*   The [`modal app rollback`](https://modal.com/docs/cli/latest/app#modal-app-rollback)
     command now accepts a `--strategy` (`rolling` or `recreate`), like [`modal deploy`](https://modal.com/docs/cli/latest/deploy)
     and [`modal app rollover`](https://modal.com/docs/cli/latest/app#modal-app-rollover)
    .
*   Sandbox connect tokens can now be scoped to a custom port ([`modal.Sandbox.create_connect_token(port=...)`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#create_connect_token)
    ).
*   It is now possible to call [`image.publish()`](https://modal.com/docs/sdk/py/latest/modal.Image#publish)
     on an Image constructed using [`modal.Image.from_id()`](https://modal.com/docs/sdk/py/latest/modal.Image#from_id)
     without first calling [`image.build()`](https://modal.com/docs/sdk/py/latest/modal.Image#build)
    .
...
This is a major release that includes several new features ([named Images](https://modal.com/docs/guide/named-images)
, version-pinned Function lookups, [Sandbox domain allowlists](https://modal.com/docs/guide/sandbox-networking)
), a new [`modal skills`](https://modal.com/docs/cli/latest/skills)
 CLI, and a small number of breaking changes.
...
*   We’re introducing a “named Image” concept, akin to a Modal-native Image registry. This feature allows you to decouple Image builds from App deployment or Sandbox creation, and it simplifies the process of sharing a canonical Image across a large number of unrelated Apps.
    *   The new SDK method [`modal.Image.publish()`](https://modal.com/docs/sdk/py/latest/modal.Image#publish)
         assigns a name to an existing Image. Names can optionally include a “tag” suffix (`"{name}:{tag}"`) to specify variants of an Image, e.g. to apply a versioning system.
    *   The new SDK method [`modal.Image.from_name()`](https://modal.com/docs/sdk/py/latest/modal.Image#from_name)
         refers directly to that Image by its name. Unlike with Image methods that specify parts of a build recipe, the [`modal.Image.from_name()`](https://modal.com/docs/sdk/py/latest/modal.Image#from_name)
         lookup will either succeed or fail with a [`modal.exception.NotFoundError`](https://modal.com/docs/sdk/py/latest/modal.exception#modalexceptionnotfounderror)
         error, but it will never trigger a build.
    *   The new [`modal image names`](https://modal.com/docs/cli/latest/image#modal-image-names)
         CLI can be used to see the current name assignments.
*   We’re introducing a “version-pinning” concept for Function lookups. By passing `version=` to [`modal.Function.from_name()`](https://modal.com/docs/sdk/py/latest/modal.Function#from_name)
     or [`modal.Cls.from_name()`](https://modal.com/docs/sdk/py/latest/modal.Cls#from_name)
    , you can retrieve an instance that will send all inputs to the same version of the Function, even after a subsequent redeployment of its App. Additionally, any transitive invocations of other Functions in the same App will also be pinned on that version. This feature makes it easy to execute workflows that involve multiple Functions when those Functions might change in incompatible ways across deployments.
*   It’s now possible to restrict the _domains_ that processes inside of a Sandbox can connect to. When you provide a list of domains to `outbound_domain_allowlist=[...]` in [`modal.Sandbox.create()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#create)
...
*   We’ve added a new [`modal skills`](https://modal.com/docs/cli/latest/skills)
     CLI for installing a foundational Modal agent skill ([`modal skills install`](https://modal.com/docs/cli/latest/skills#modal-skills-install)
    ) and keeping it up to date over time ([`modal skills update`](https://modal.com/docs/cli/latest/skills#modal-skills-update)
    ). Please help us improve it by sharing any feedback you have about its impact on agentic development with Modal.
*   We’ve added a new [`modal.Workspace`](https://modal.com/docs/sdk/py/latest/modal.Workspace)
     object for programmatic interaction with your Workspace configuration. The initial release offers a [`workspace.members.list()`](https://modal.com/docs/sdk/py/latest/modal.Workspace#memberslist)
     method; expect more features soon.
*   The [`modal`](https://modal.com/docs/cli/latest)
     CLI now normalizes the keys of its `--json` output by lowercasing and substituting underscores for non-alphanumeric characters.
*   We’ve added a new [`sandbox.filesystem.watch()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#filesystemwatch)
     method to the Sandbox Filesystem API, and we’ve deprecated the alpha [`modal.Sandbox.watch()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#watch)
     method. The new method has a different implementation and offers significantly improved latency and reliability.
*   We’ve made two small breaking changes around Sandbox snapshots:
    *   [`modal.Sandbox.snapshot_filesystem()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#snapshot_filesystem)
         and [`modal.Sandbox.snapshot_directory()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#snapshot_directory)
         now accept an explicit `ttl=` keyword argument that configures a retention interval (in seconds) for the resulting Image. Both methods default to 30 days (`ttl=30 * 24 * 3600`). **Note:** this alters the default behavior of these methods, because snapshot Images were previously persisted indefinitely. Pass an explicit `ttl=None` to retain the previous behavior.
    *   [`modal.Sandbox.snapshot_directory()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#snapshot_directory)
         now also accepts a `timeout=` keyword argument (defaulting to `55` seconds), which brings it to parity with [`modal.Sandbox.snapshot_filesystem()`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#snapshot_filesystem)
        . If the snapshot does not return before the deadline, a [`modal.exception.TimeoutError`](https://modal.com/docs/sdk/py/latest/modal.exception#modalexceptiontimeouterror)
         will be raised. **Note:** this alters the default behavior, which previously waited indefinitely, but you can set an arbitrarily long timeout.
*   We’ve removed several deprecated static methods (`.delete()` and `.create_deployed()`) on Modal storage objects ([`modal.Volume`](https://modal.com/docs/sdk/py/latest/modal.Volume)
    , etc.). Use the [`.objects.delete()`](https://modal.com/docs/sdk/py/latest/modal.Volume#objectsdelete)
     and [`.objects.create()`](https://modal.com/docs/sdk/py/latest/modal.Volume#objectscreate)
     methods instead.
...
*   We’ve added a new `modal.Environment` object for programmatically managing Environments, and we’ve expanded the `modal environment` CLI to support [RBAC](https://modal.com/docs/guide/rbac)
     configuration.
...
*   It’s now possible to place restrictions on _inbound_ Sandbox connections by setting `inbound_cidr_allowlist=[...]` in `modal.Sandbox.create()`. We are also adding a new `outbound_cidr_allowlist=[...]` parameter and deprecating the existing `cidr_allowlist=[...]` to avoid confusion.
...
We’re releasing a new [Sandbox filesystem API](https://modal.com/docs/guide/sandbox-files)
 (currently in Beta) with significantly improved reliability and ergonomics:
...
*   An exception is now raised if the _same_ Volume or CloudBucketMount is mounted at multiple paths in a container.
...
*   Images built with `modal.Image.micromamba()` using the 2023.12 [Image Builder Version](https://modal.com/docs/guide/images#image-builder-updates)
     will now use a Python version that matches their local environment by default, rather than defaulting to Python 3.9.
...
*   `modal.CloudBucketMount` now supports `force_path_style=True` to disable virtual-host-style addressing. See [mountpoint-s3 endpoints docs](https://github.com/awslabs/mountpoint-s3/blob/main/doc/CONFIGURATION.md#endpoints-and-aws-privatelink)
     for details.
...
*   The new `modal.Sandbox.create_connect_token()` method facilitates authentication for making HTTP / Websocket requests to a server running in a Sandbox:
    ...
    See the [Sandbox Networking guide](https://modal.com/docs/guide/sandbox-networking)
     for more information.
...
This release introduces support for the `2025.06` [Image Builder Version](https://modal.com/docs/guide/images#image-builder-updates)
, which is in a “preview” state. The new image builder includes several major changes to how the Modal client dependencies are included in Modal Images. These improvements should greatly reduce the risk of conflicts with user code dependencies. They also allow Modal Sandboxes to easily be used with existing Images or Dockerfiles that are not themselves compatible with the Modal client library. You can see more details and update your Workspace on its [Image Config](https://modal.com/settings/image-config)
 page. Please share any issues that you encounter as we work to make the version stable.

We’re also introducing first-class support for building Modal Images with the [uv package manager](https://docs.astral.sh/uv/)
 through the new [`modal.Image.uv_pip_install`](https://modal.com/docs/sdk/py/latest/modal.Image#uv_pip_install)
 and [`modal.Image.uv_sync`](https://modal.com/docs/sdk/py/latest/modal.Image#uv_sync)
 methods:
...
*   Added a [`modal.Sandbox.reload_volumes`](https://modal.com/docs/sdk/py/latest/modal.Sandbox#reload_volumes)
...
*   Added a [`modal.Volume.read_only`](https://modal.com/docs/sdk/py/latest/modal.Volume#read_only)
...
With this release, we’re beginning to enforce the deprecations discussed in the [1.0 migration guide](https://modal.com/docs/guide/modal-1-0-migration)
. Going forward, we’ll include breaking changes for outstanding deprecations in `1.Y.0` releases, so we recommend pinning Modal on a minor version (`modal~=1.0.0`) if you have not addressed the existing warnings. While we’ll continue to make improvements to the Modal API, new deprecations will be introduced at a substantially reduced rate, and support windows for older client versions will lengthen.
...
[Changelog](https://modal.com/docs/sdk/py/changelog#changelog)
[Latest](https://modal.com/docs/sdk/py/changelog#latest)
[1.5.1 (2026-06-23)](https://modal.com/docs/sdk/py/changelog#151-2026-06-23)
[1.5.0 (2026-06-09)](https://modal.com/docs/sdk/py/changelog#150-2026-06-09)
[1.4](https://modal.com/docs/sdk/py/changelog#14)
[1.4.3 (2026-05-18)](https://modal.com/docs/sdk/py/changelog#143-2026-05-18)
[1.4.2 (2026-04-16)](https://modal.com/docs/sdk/py/changelog#142-2026-04-16)
[1.4.1 (2026-03-30)](https://modal.com/docs/sdk/py/changelog#141-2026-03-30)
[1.4.0 (2026-03-25)](https://modal.com/docs/sdk/py/changelog#140-2026-03-25)
[1.3](https://modal.com/docs/sdk/py/changelog#13)
[1.3.5 (2026-03-03)](https://modal.com/docs/sdk/py/changelog#135-2026-03-03)
[1.3.4 (2026-02-23)](https://modal.com/docs/sdk/py/changelog#134-2026-02-23)
[1.3.3 (2026-02-12)](https://modal.com/docs/sdk/py/changelog#133-2026-02-12)
[1.3.2 (2026-01-30)](https://modal.com/docs/sdk/py/changelog#132-2026-01-30)
[1.3.1 (2026-01-22)](https://modal.com/docs/sdk/py/changelog#131-2026-01-22)
[1.3.0 (2025-12-19)](https://modal.com/docs/sdk/py/changelog#130-2025-12-19)
[1.2](https://modal.com/docs/sdk/py/changelog#12)
[1.2.6 (2025-12-16)](https://modal.com/docs/sdk/py/changelog#126-2025-12-16)
[1.2.5 (2025-12-12)](https://modal.com/docs/sdk/py/changelog#125-2025-12-12)
[1.2.4 (2025-11-21)](https://modal.com/docs/sdk/py/changelog#124-2025-11-21)
[1.2.3 (2025-11-20)](https://modal.com/docs/sdk/py/changelog#123-2025-11-20)
[1.2.2 (2025-11-10)](https://modal.com/docs/sdk/py/changelog#122-2025-11-10)
[1.2.1 (2025-10-22)](https://modal.com/docs/sdk/py/changelog#121-2025-10-22)
[1.2.0 (2025-10-09)](https://modal.com/docs/sdk/py/changelog#120-2025-10-09)
[1.1](https://modal.com/docs/sdk/py/changelog#11)
[1.1.4 (2025-09-03)](https://modal.com/docs/sdk/py/changelog#114-2025-09-03)
[1.1.3 (2025-08-19)](https://modal.com/docs/sdk/py/changelog#113-2025-08-19)
[1.1.2 (2025-08-14)](https://modal.com/docs/sdk/py/changelog#112-2025-08-14)
[1.1.1 (2025-08-01)](https://modal.com/docs/sdk/py/changelog#111-2025-08-01)
[1.1.0 (2025-07-17)](https://modal.com/docs/sdk/py/changelog#110-2025-07-17)
[1.0](https://modal.com/docs/sdk/py/changelog#10)
[1.0.5 (2025-06-27)](https://modal.com/docs/sdk/py/changelog#105-2025-06-27)
[1.0.4 (2025-06-13)](https://modal.com/docs/sdk/py/changelog#104-2025-06-13)
[1.0.3 (2025-06-05)](https://modal.com/docs/sdk/py/changelog#103-2025-06-05)
[1.0.2 (2025-05-26)](https://modal.com/docs/sdk/py/changelog#102-2025-05-26)
[1.0.1 (2025-05-19)](https://modal.com/docs/sdk/py/changelog#101-2025-05-19)
[1.0.0 (2025-05-16)](https://modal.com/docs/sdk/py/changelog#100-2025-05-16)
