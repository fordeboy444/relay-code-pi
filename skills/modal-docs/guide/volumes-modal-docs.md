...
- **Summary:** Volumes are a high-performance distributed file system for Modal applications. They are optimized for write-once, read-many I/O workloads, like creating machine learning model weights and distributing them for inference.
...
This page is a high-level guide to using Modal Volumes. For reference documentation on the `modal.Volume` object, see [this page](https://modal.com/docs/sdk/py/latest/modal.Volume)
. For reference documentation on the `modal volume` CLI command, see [this page](https://modal.com/docs/cli/latest/volume)
.

Pricing 

Please refer to our [pricing page](https://modal.com/pricing)
 for up-to-date prices. We calculate usage by snapshotting your total storage once a day. When you delete data, you may still be billed for that storage for up to four days, to reflect our underlying processing costs.

Volumes v2 

Read more about [Volumes v2](https://modal.com/docs/guide/volumes#volumes-v2-overview)
 below.

Creating a Volume 

The easiest way to create a Volume and use it as a part of your App is to use the [`modal volume create`](https://modal.com/docs/cli/latest/volume#modal-volume-create)
...
To attach an existing Volume to a Modal Function, use [`Volume.from_name`](https://modal.com/docs/sdk/py/latest/modal.Volume#from_name)
:
...
When attaching a Volume to a Function or Sandbox, you can configure mount options using [`Volume.with_mount_options`](https://modal.com/docs/sdk/py/latest/modal.Volume#with_mount_options)
. These options are not stored on the Volume itself — they apply per container mount, so the same Volume can be mounted differently for distinct containers.
...
Volumes can also be used outside Modal via the [Python SDK](https://modal.com/docs/sdk/py/latest/modal.Volume#modalvolume)
 or our [CLI](https://modal.com/docs/cli/latest/volume)
.
...
For more details, see the [reference documentation](https://modal.com/docs/sdk/py/latest/modal.Volume)
.
...
Unlike a normal filesystem, you need to explicitly reload the Volume to see changes made since it was first mounted. This reload is handled by invoking the [`.reload()`](https://modal.com/docs/sdk/py/latest/modal.Volume#reload)
 method on a Volume object. Similarly, any Volume changes made within a container need to be committed for those the changes to become visible outside the current container. This is handled periodically by [background commits](https://modal.com/docs/guide/volumes#background-commits)
 and directly by invoking the [`.commit()`](https://modal.com/docs/sdk/py/latest/modal.Volume#commit)
 method on a `modal.Volume` object.

At container creation time the latest state of an attached Volume is mounted. If the Volume is then subsequently modified by a commit operation in another running container, that Volume modification won’t become available until the original container does a [`.reload()`](https://modal.com/docs/sdk/py/latest/modal.Volume#reload)
.
...
Being able to persist changes to Volumes without changing your application code is especially useful when [training or fine-tuning models using frameworks](https://modal.com/docs/guide/volumes#model-checkpointing)
.

Model serving 

A single ML model can be served by simply baking it into a `modal.Image` at build time using [`run_function`](https://modal.com/docs/sdk/py/latest/modal.Image#run_function)
...
To periodically checkpoint into a `modal.Volume`, just set the `Trainer`’s [`output_dir`](https://huggingface.co/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments.output_dir)
...
You can only reload a Volume when there no open files on the Volume. If you have open files on the Volume the [`.reload()`](https://modal.com/docs/sdk/py/latest/modal.Volume#reload)
...
Note, however, that concurrent access to a particular _file_ in a Volume still has last-write-wins semantics in many circumstances. These semantics are unacceptable for most applications, so any particular file should only be written to by a single container at a time.
...
This is useful when you don’t have access to the Python SDK’s [`.commit()`](https://modal.com/docs/sdk/py/latest/modal.Volume#commit)
 method, such as when running shell commands in a Sandbox or during an interactive `modal shell` session.
...
*   [Character LoRA fine-tuning](https://modal.com/docs/examples/diffusers_lora_finetune)
     with model storage on a Volume
*   [Protein folding](https://modal.com/docs/examples/chai1)
     with model weights and output files stored on Volumes
*   [Dataset visualization with Datasette](https://modal.com/docs/examples/cron_datasette)
     using a SQLite database on a Volume

[Volumes](https://modal.com/docs/guide/volumes#volumes)
[Pricing](https://modal.com/docs/guide/volumes#pricing)
[Volumes v2](https://modal.com/docs/guide/volumes#volumes-v2)
[Creating a Volume](https://modal.com/docs/guide/volumes#creating-a-volume)
[Using a Volume on Modal](https://modal.com/docs/guide/volumes#using-a-volume-on-modal)
[Mount options](https://modal.com/docs/guide/volumes#mount-options)
[Read-only mounts](https://modal.com/docs/guide/volumes#read-only-mounts)
[Mounting a sub-path](https://modal.com/docs/guide/volumes#mounting-a-sub-path)
[Downloading a file from a Volume](https://modal.com/docs/guide/volumes#downloading-a-file-from-a-volume)
[Creating Volumes lazily from code](https://modal.com/docs/guide/volumes#creating-volumes-lazily-from-code)
[Using a Volume from outside of Modal](https://modal.com/docs/guide/volumes#using-a-volume-from-outside-of-modal)
[Using a Volume from local code](https://modal.com/docs/guide/volumes#using-a-volume-from-local-code)
[Using a Volume via the command line](https://modal.com/docs/guide/volumes#using-a-volume-via-the-command-line)
...
[Model serving](https://modal.com/docs/guide/volumes#model-serving)
[Model checkpointing](https://modal.com/docs/guide/volumes#model-checkpointing)
[Hugging Face transformers](https://modal.com/docs/guide/volumes#hugging-face-transformers)
[Volume performance](https://modal.com/docs/guide/volumes#volume-performance)
[Filesystem consistency](https://modal.com/docs/guide/volumes#filesystem-consistency)
[Concurrent modification](https://modal.com/docs/guide/volumes#concurrent-modification)
[Busy Volume errors](https://modal.com/docs/guide/volumes#busy-volume-errors)
[Can’t find file on Volume errors](https://modal.com/docs/guide/volumes#cant-find-file-on-volume-errors)
[Disk usage reporting](https://modal.com/docs/guide/volumes#disk-usage-reporting)
[Volumes v2 overview](https://modal.com/docs/guide/volumes#volumes-v2-overview)
[Volumes v2 are still in Beta](https://modal.com/docs/guide/volumes#volumes-v2-are-still-in-beta)
[Volumes v2 are HIPAA compliant](https://modal.com/docs/guide/volumes#volumes-v2-are-hipaa-compliant)
[Volumes v2 is more scaleable](https://modal.com/docs/guide/volumes#volumes-v2-is-more-scaleable)
[In v2, you can store as many files as you want](https://modal.com/docs/guide/volumes#in-v2-you-can-store-as-many-files-as-you-want)
[In v2, you can write concurrently from hundreds of containers](https://modal.com/docs/guide/volumes#in-v2-you-can-write-concurrently-from-hundreds-of-containers)
[In v2, random accesses have improved performance](https://modal.com/docs/guide/volumes#in-v2-random-accesses-have-improved-performance)
[In v2, you can commit using sync](https://modal.com/docs/guide/volumes#in-v2-you-can-commit-using-sync)
[Volumes v2 has a few limits in place](https://modal.com/docs/guide/volumes#volumes-v2-has-a-few-limits-in-place)
[Upgrading v1 Volumes](https://modal.com/docs/guide/volumes#upgrading-v1-volumes)
[Further examples](https://modal.com/docs/guide/volumes#further-examples)

See it in action

[Fine-tuning and serving custom image generation models](https://modal.com/docs/examples/diffusers_lora_finetune)

[Folding proteins](https://modal.com/docs/examples/chai1)

[Serving interactive visualizations for a SQLite database](https://modal.com/docs/examples/cron_datasette)
