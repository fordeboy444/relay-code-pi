...
- **Summary:** Modal runs containers globally across multiple different clouds. By default, all inputs to Modal Functions are routed through our servers in Virginia, USA (us-east) before being sent to a container for execution.
...
A multiplier on top of our [base usage pricing](https://modal.com/pricing)
 will be applied to any Function or Sandbox that has a container region defined.
...
[Inputs and outputs larger than 2 MiB](https://modal.com/docs/guide/security#function-inputs-and-outputs)
 are still uploaded to object storage in `us-east`.
...
[Cloudping.co](https://www.cloudping.co/)
 provides good estimates of the latency between regions. For example, the round-trip latency between AWS `us-east` (Virginia, USA) and `us-west` (Oregon, USA) is around 60ms.
...
To optimize latency further, please contact us on [Slack](https://modal.com/slack)
 or at [support@modal.com](mailto:support@modal.com)
.

[Region selection](https://modal.com/docs/guide/region-selection#region-selection)
[Specifying a container region](https://modal.com/docs/guide/region-selection#specifying-a-container-region)
[Pricing](https://modal.com/docs/guide/region-selection#pricing)
[Container region options](https://modal.com/docs/guide/region-selection#container-region-options)
[Regional routing](https://modal.com/docs/guide/region-selection#regional-routing)
[Specifying a routing region](https://modal.com/docs/guide/region-selection#specifying-a-routing-region)
[Current restrictions](https://modal.com/docs/guide/region-selection#current-restrictions)
[Optimizing latency](https://modal.com/docs/guide/region-selection#optimizing-latency)
