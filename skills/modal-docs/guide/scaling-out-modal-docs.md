# Scaling out | Modal Docs

- **URL:** https://modal.com/docs/guide/scale
- **Summary:** Modal makes it trivially easy to scale compute across thousands of containers. You won’t have to worry about your App crashing if it goes viral or need to wait a long time for your batch jobs to complete.
...
It’s also possible to update the autoscaler settings dynamically (i.e., without redeploying the App) using the [`Function.update_autoscaler()`](https://modal.com/docs/sdk/py/latest/modal.Function#update_autoscaler)
 method:

    f = modal.Function.from_name("my-app", "f")
    f.update_autoscaler(max_containers=100)

The autoscaler settings will revert to the configuration in the function decorator the next time you deploy the App. Or they can be overridden by further dynamic updates:
...
When you have a [`modal.Cls`](https://modal.com/docs/sdk/py/latest/modal.Cls)
, `update_autoscaler` is a method on an _instance_ and will control the autoscaling behavior of containers serving the Function with that specific set of parameters:
...
Note that it’s necessary to disable type checking on this line, because the object will appear as an instance of the class that you defined rather than the Modal wrapper type.

Parallel execution of inputs 

If your code is running the same function repeatedly with different independent inputs (e.g., a grid search), the easiest way to increase performance is to run those function calls in parallel using Modal’s [`Function.map()`](https://modal.com/docs/sdk/py/latest/modal.Function#map)
 method.
...
By default, if any of the function calls raises an exception, the exception will be propagated. To treat exceptions as successful results and aggregate them in the results list, pass in [`return_exceptions=True`](https://modal.com/docs/sdk/py/latest/modal.Function#map)
.
...
If your function takes multiple variable arguments, you can either use [`Function.map()`](https://modal.com/docs/sdk/py/latest/modal.Function#map)
 with one input iterator per argument, or [`Function.starmap()`](https://modal.com/docs/sdk/py/latest/modal.Function#starmap)
...
Sometimes you can speed up your applications by utilizing GPU acceleration. See the [GPU section](https://modal.com/docs/guide/gpu)
...
[Scaling out](https://modal.com/docs/guide/scale#scaling-out)
[How does autoscaling work on Modal?](https://modal.com/docs/guide/scale#how-does-autoscaling-work-on-modal)
[Configuring autoscaling behavior](https://modal.com/docs/guide/scale#configuring-autoscaling-behavior)
[Dynamic autoscaler updates](https://modal.com/docs/guide/scale#dynamic-autoscaler-updates)
[Parallel execution of inputs](https://modal.com/docs/guide/scale#parallel-execution-of-inputs)
[Exceptions](https://modal.com/docs/guide/scale#exceptions)
[Starmap](https://modal.com/docs/guide/scale#starmap)
[Gotchas](https://modal.com/docs/guide/scale#gotchas)
[Asynchronous usage](https://modal.com/docs/guide/scale#asynchronous-usage)
[GPU acceleration](https://modal.com/docs/guide/scale#gpu-acceleration)
[Scaling Limits](https://modal.com/docs/guide/scale#scaling-limits)

See it in action

[Auto-scaling LLM inference endpoints](https://modal.com/docs/examples/llm_inference)

[Job queue for OCR](https://modal.com/docs/examples/doc_ocr_jobs)

[Parallel web scraping](https://modal.com/docs/examples/webscraper#scaling-out)
