...
- **URL:** https://modal.com/docs/guide/dynamic-batching
- **Summary:** Modal’s @batched feature allows you to accumulate requests and process them in dynamically-sized batches, rather than one-by-one.
...
Static batching can lead to unbounded latency, as the function waits for a fixed number of requests to arrive. Modal’s dynamic batching waits for the lesser of a fixed time _or_ a fixed number of requests before executing, maximizing the throughput benefit of batching while minimizing the latency penalty.

Enable dynamic batching with `@batched` 

To enable dynamic batching, apply the [`@modal.batched` decorator](https://modal.com/docs/sdk/py/latest/modal.batched)
 to the target Python function. Then, wrap it in `@app.function()` and run it on Modal, and the inputs will be accumulated and processed in batches.
...
Methods on Modal [`Cls`](https://modal.com/docs/guide/lifecycle-functions)
...
*   If a class has a Batched Method, it **cannot have other Batched Methods or [Methods](https://modal.com/docs/sdk/py/latest/modal.method#modalmethod)
    **.
...
Here’s a simple example of serving a Function that batches requests dynamically with a [`@modal.fastapi_endpoint`](https://modal.com/docs/guide/webhooks)
. Run [`modal serve`](https://modal.com/docs/cli/latest/serve)
, submit requests to the endpoint, and the Function will batch your requests on the fly.
...
Now, you can submit requests to the Web Function and process them in batches. For instance, the three requests in the following example, which might be requests from concurrent clients in a real deployment, will be batched into two executions:
...
[Dynamic batching](https://modal.com/docs/guide/dynamic-batching#dynamic-batching)
[Enable dynamic batching with @batched](https://modal.com/docs/guide/dynamic-batching#enable-dynamic-batching-with-batched)
[Use @batched with functions that take and return lists](https://modal.com/docs/guide/dynamic-batching#use-batched-with-functions-that-take-and-return-lists)
[Modal Cls methods are compatible with dynamic batching](https://modal.com/docs/guide/dynamic-batching#modal-cls-methods-are-compatible-with-dynamic-batching)
[Configure the wait time and batch size of dynamic batches](https://modal.com/docs/guide/dynamic-batching#configure-the-wait-time-and-batch-size-of-dynamic-batches)
[Selecting a batch configuration](https://modal.com/docs/guide/dynamic-batching#selecting-a-batch-configuration)
[Serve Web Functions with dynamic batching](https://modal.com/docs/guide/dynamic-batching#serve-web-functions-with-dynamic-batching)
