# Asynchronous API usage | Modal Docs

- **URL:** https://modal.com/docs/guide/async
- **Summary:** All of the functions in Modal are available in both standard (blocking) and asynchronous variants. The async interface can be accessed by appending .aio to any function in the Modal API.
...
For example, instead of `my_modal_function.remote("hello")` in a blocking context, you can use `await my_modal_function.remote.aio("hello")` to get an asynchronous coroutine response, for use with Python’s `asyncio` library.
...
[Asynchronous API usage](https://modal.com/docs/guide/async#asynchronous-api-usage)
[Async functions](https://modal.com/docs/guide/async#async-functions)
