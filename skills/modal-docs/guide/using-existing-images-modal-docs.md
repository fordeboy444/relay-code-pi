# Using existing images | Modal Docs

- **URL:** https://modal.com/docs/guide/existing-images
- **Summary:** This guide walks you through how to use an existing container image as a Modal Image.
...
To load an image from a public registry, just pass the image name, including any tags, to [`Image.from_registry`](https://modal.com/docs/sdk/py/latest/modal.Image#from_registry)
:
...
*   The image has a [compatible `ENTRYPOINT`](https://modal.com/docs/guide/existing-images#entrypoint)
    

Additionally, to be used with a Modal Function, the image needs to have `python` and `pip` installed and available on the `$PATH`. If an existing image does not have either `python` or `pip` set up compatibly, you can still use it. Just provide a version number as the `add_python` argument to install a reproducible [standalone build](https://github.com/indygreg/python-build-standalone)
 of Python:
...
There are some additional restrictions for older versions of the Modal image builder. Image builder version is set at a workspace level via the settings page [here](https://modal.com/settings/image-builder-version)
...
To pull container images from private Docker Hub repositories, [create an access token](https://docs.docker.com/security/for-developers/access-tokens/)
 with “Read-Only” permissions and use this token value and your Docker Hub username to create a Modal [Secret](https://modal.com/docs/guide/secrets)
.

    REGISTRY_USERNAME=my-dockerhub-username
    REGISTRY_PASSWORD=<your-dockerhub-access-token>

Use this Secret with the [`modal.Image.from_registry`](https://modal.com/docs/sdk/py/latest/modal.Image#from_registry)
 method.
...
Images on ECR must be private and follow [image configuration requirements](https://modal.com/docs/sdk/py/latest/modal.Image#from_aws_ecr)
.

### Google Artifact Registry and Google Container Registry 

For further detail on how to pull images from Google’s image registries, see [`modal.Image.from_gcp_artifact_registry`](https://modal.com/docs/sdk/py/latest/modal.Image#from_gcp_artifact_registry)
.

### Azure Container Registry (ACR) 

Modal doesn’t have native Azure support, but you can pull images from a private ACR using ACR’s [token-based repository permissions](https://learn.microsoft.com/en-us/azure/container-registry/container-registry-token-based-repository-permissions)
 to generate long-lived Docker credentials. Those credentials (token and password) can then be stored as a Modal Secret and used with [`modal.Image.from_registry`](https://modal.com/docs/sdk/py/latest/modal.Image#from_registry)
 the same way as [Docker Hub private registry](https://modal.com/docs/guide/existing-images#docker-hub-private)
 credentials.

Bring your own image definition with `.from_dockerfile` 

You can define an Image from an existing Dockerfile by passing its path to [`Image.from_dockerfile`](https://modal.com/docs/sdk/py/latest/modal.Image#from_dockerfile)
:
...
Note that you can still extend this Image using image builder methods! See [the guide](https://modal.com/docs/guide/images)
 for details.
...
If the above file is saved as `/usr/bin/my_entrypoint.sh` in your container, then you can register it as an entrypoint with `ENTRYPOINT ["/usr/bin/my_entrypoint.sh"]` in your Dockerfile, or with [`entrypoint`](https://modal.com/docs/sdk/py/latest/modal.Image#entrypoint)
...
[Using existing images](https://modal.com/docs/guide/existing-images#using-existing-images)
[Load an image from a public registry with .from_registry](https://modal.com/docs/guide/existing-images#load-an-image-from-a-public-registry-with-from_registry)
[Load images from private registries](https://modal.com/docs/guide/existing-images#load-images-from-private-registries)
[Docker Hub (Private)](https://modal.com/docs/guide/existing-images#docker-hub-private)
[Elastic Container Registry (ECR)](https://modal.com/docs/guide/existing-images#elastic-container-registry-ecr)
[Google Artifact Registry and Google Container Registry](https://modal.com/docs/guide/existing-images#google-artifact-registry-and-google-container-registry)
[Azure Container Registry (ACR)](https://modal.com/docs/guide/existing-images#azure-container-registry-acr)
[Bring your own image definition with .from_dockerfile](https://modal.com/docs/guide/existing-images#bring-your-own-image-definition-with-from_dockerfile)
[Dockerfile command compatibility](https://modal.com/docs/guide/existing-images#dockerfile-command-compatibility)
[ENTRYPOINT](https://modal.com/docs/guide/existing-images#entrypoint)
[ENV](https://modal.com/docs/guide/existing-images#env)

See it in action

[Registry image for Algolia indexing](https://modal.com/docs/examples/algolia_indexer)
