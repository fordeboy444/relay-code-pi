...
description: Use when the user wants to design or spec a new Modal + Trigger.dev (relay-code) automation before any code is written. Triggers on "design an automation", "spec out a new task", "I want to add an automation", or /relay-brainstorm. Do not use when a spec already exists in docs/specs/ — use /relay-plan instead.
---
...
This is the first phase of the three-skill flow: **brainstorm → plan → execute**. It produces the *what* and *why* (the spec). `/relay-plan` then produces the *how* (the plan); `/relay-execute` builds it.

## Input

The user should describe the automation they have in mind. If they only typed `/relay-brainstorm`, ask (via the AskUserQuestion tool, provided by rpiv-ask-user-question):
...
9. **Hand off** — once the spec is approved, do **not** auto-invoke the next skill. Stop and tell the user:
   > The spec is written to `docs/specs/<slug>.md`. Run `/clear` to clear the context, then run `/relay-plan <slug>` to write the implementation plan.
...
- **HARD-GATE:** never write application code (`src/*`, `modal_bridge.py`, `scripts/*`) during brainstorm — only the spec. The convention files (`src/config.ts`, `src/schema.ts`, `modal_bridge.py`, `src/trigger/*.ts`) are mutated **only** by the `relay_add_*` tools during `/relay-execute` — never hand-edit them.
...
- Keep the spec concise but complete enough that `/relay-plan` can produce a plan from it without re-asking.