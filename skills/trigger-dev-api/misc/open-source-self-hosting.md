...
This is a legacy guide for self-hosting v3 using Docker, you can find the v4 guide [here](https://trigger.dev/docs/self-hosting/docker)
.
...
[​](https://trigger.dev/docs/open-source-self-hosting#overview)

Overview
---------------------------------------------------------------------------

![Self-hosting architecture](https://mintcdn.com/trigger/5SxX7bFjJKRsidSL/images/self-hosting.png?w=2500&fit=max&auto=format&n=5SxX7bFjJKRsidSL&q=85&s=86b4df1644f0d2ab7226937dffcf5c36)

The self-hosting guide covers two alternative setups. The first option uses a simple setup where you run everything on one server. With the second option, the webapp and worker components are split on two separate machines. You’re going to need at least one Debian (or derivative) machine with Docker and Docker Compose installed. We’ll also use Ngrok to expose the webapp to the internet.

[​](https://trigger.dev/docs/open-source-self-hosting#support)
...
This guide outlines a quick way to start self-hosting Trigger.dev for evaluation purposes - it won’t result in a production-ready deployment. Security, scaling, and reliability concerns are not fully addressed here. As self-hosted deployments tend to have unique requirements and configurations, we don’t provide specific advice for securing your deployment, scaling up, or improving reliability. Should the burden ever get too much, we’d be happy to see you on [Trigger.dev cloud](https://trigger.dev/pricing)
 where we deal with these concerns for you.

Please consider these additional warnings

*   The [docker checkpoint](https://docs.docker.com/reference/cli/docker/checkpoint/)
     command is an experimental feature which may not work as expected. It won’t be enabled by default. Instead, the containers will stay up and their processes frozen. They won’t consume CPU but they _will_ consume RAM.
...
[​](https://trigger.dev/docs/open-source-self-hosting#requirements)
...
[​](https://trigger.dev/docs/open-source-self-hosting#option-1-single-server)
...
[​](https://trigger.dev/docs/open-source-self-hosting#server-setup)
...
[​](https://trigger.dev/docs/open-source-self-hosting#trigger-dev-setup)
...
[​](https://trigger.dev/docs/open-source-self-hosting#manual)
...
[​](https://trigger.dev/docs/open-source-self-hosting#tunnelling)
...
[​](https://trigger.dev/docs/open-source-self-hosting#registry-setup)

Registry setup

If you want to deploy v3 projects, you will need access to a Docker registry. The [CLI deploy](https://trigger.dev/docs/cli-deploy-commands)
...
3.  Log in to Docker Hub both locally and your server. For the split setup, this will be the worker machine. You may want to create an [access token](https://hub.docker.com/settings/security)
     for this.
...
[​](https://trigger.dev/docs/open-source-self-hosting#option-2-split-services)
...
[​](https://trigger.dev/docs/open-source-self-hosting#webapp-setup)
...
2.  **Tunnelling.** This is now _required_. Please follow the [tunnelling](https://trigger.dev/docs/open-source-self-hosting#tunnelling)
     section.

### 

[​](https://trigger.dev/docs/open-source-self-hosting#worker-setup)
...
4.  **Registry setup.** Follow the [registry setup](https://trigger.dev/docs/open-source-self-hosting#registry-setup)
...
[​](https://trigger.dev/docs/open-source-self-hosting#additional-features)
...
[​](https://trigger.dev/docs/open-source-self-hosting#large-payloads)

Large payloads

By default, payloads over 512KB will be offloaded to S3-compatible storage. If you don’t provide the required env vars, runs with payloads larger than this will fail. For example, using Cloudflare R2:
...
[​](https://trigger.dev/docs/open-source-self-hosting#version-locking)
...
[​](https://trigger.dev/docs/open-source-self-hosting#auth-options)
...
[​](https://trigger.dev/docs/open-source-self-hosting#checkpoint-support)

Checkpoint support

This requires an _experimental Docker feature_. Successfully checkpointing a task today, does not mean you will be able to restore it tomorrow. Your data may be lost. You’ve been warned!

Checkpointing allows you to save the state of a running container to disk and restore it later. This can be useful for long-running tasks that need to be paused and resumed without losing state. Think fan-out and fan-in, or long waits in email campaigns. The checkpoints will be pushed to the same registry as the deployed images. Please see the [registry setup](https://trigger.dev/docs/open-source-self-hosting#registry-setup)
 section for more information.

#### 

[​](https://trigger.dev/docs/open-source-self-hosting#requirements-2)
...
[​](https://trigger.dev/docs/open-source-self-hosting#setup)

Setup

Underneath the hood this uses Checkpoint and Restore in Userspace, or [CRIU](https://github.com/checkpoint-restore/criu)
...
[​](https://trigger.dev/docs/open-source-self-hosting#updating)
...
[​](https://trigger.dev/docs/open-source-self-hosting#from-beta)
...
*   **Pull in updates.** We’ve added a new container for [Electric](https://github.com/electric-sql/electric)
     and made some other improvements. Run `git pull` to get the latest updates.
...
[​](https://trigger.dev/docs/open-source-self-hosting#troubleshooting)
...
[​](https://trigger.dev/docs/open-source-self-hosting#cli-usage)
...
[​](https://trigger.dev/docs/open-source-self-hosting#login)

Login

To avoid being redirected to the [Trigger.dev Cloud](https://cloud.trigger.dev/)
 login page when using the CLI, you can specify the URL of your self-hosted instance with the `--api-url` or `-a` flag. For example:
...
[​](https://trigger.dev/docs/open-source-self-hosting#custom-profiles)
...
[​](https://trigger.dev/docs/open-source-self-hosting#verify-login)
...
[​](https://trigger.dev/docs/open-source-self-hosting#deploy)

Deploy

On [Trigger.dev Cloud](https://cloud.trigger.dev/)
, we build deployments remotely and push those images for you. When self-hosting you will have to do that locally yourself. This can be done with the `--self-hosted` and `--push` flags. For example:
...
[​](https://trigger.dev/docs/open-source-self-hosting#ci-/-github-actions)
...
[​](https://trigger.dev/docs/open-source-self-hosting#telemetry)
...
[Previous](https://trigger.dev/docs/self-hosting/env/supervisor)
[ContributingYou can contribute to Trigger.dev in many ways.\
\
Next](https://trigger.dev/docs/open-source-contributing)
