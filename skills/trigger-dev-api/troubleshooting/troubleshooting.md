...
[​](https://trigger.dev/docs/troubleshooting#development)
...
[​](https://trigger.dev/docs/troubleshooting#eacces-permission-denied)
...
[​](https://trigger.dev/docs/troubleshooting#clear-the-build-cache)
...
[​](https://trigger.dev/docs/troubleshooting#yarn-plug%E2%80%99n%E2%80%99play-conflicts)
...
[​](https://trigger.dev/docs/troubleshooting#connection-error-when-logging-in)
...
[​](https://trigger.dev/docs/troubleshooting#runs-never-dequeue-in-dev)
...
[​](https://trigger.dev/docs/troubleshooting#deployment)

Deployment
----------------------------------------------------------------------

Running the \[trigger.dev deploy\] command builds and deploys your code. Sometimes there can be issues building your code. You can run the deploy command with `--log-level debug` at the end. This will spit out a lot of information about the deploy. If you can’t figure out the problem from the information below please join [our Discord](https://trigger.dev/discord)
 and create a help forum post. Do NOT share the extended debug logs publicly as they might reveal private information about your project. You can also review the build by supplying the `--dry-run` flag. This will build your project but not deploy it. You can then inspect the build output on your machine. Here are some common problems and their solutions:

### 

[​](https://trigger.dev/docs/troubleshooting#failed-to-build-project-image-error-building-image)
...
There should be a link below the error message to the full build logs on your machine. Take a look at these to see what went wrong. Join [our Discord](https://trigger.dev/discord)
 and you share it privately with us if you can’t figure out what’s going wrong. Do NOT share these publicly as the verbose logs might reveal private information about your project.

### 

[​](https://trigger.dev/docs/troubleshooting#error-failed-to-solve-failed-to-resolve-source-metadata-for-docker-io/docker/dockerfile1)
...
[​](https://trigger.dev/docs/troubleshooting#deployment-encountered-an-error)
...
[​](https://trigger.dev/docs/troubleshooting#resource_exhausted)
...
[​](https://trigger.dev/docs/troubleshooting#no-loader-is-configured-for--node-files)
...
[​](https://trigger.dev/docs/troubleshooting#cannot-find-module-/app/lib/worker-js-when-using-pino)
...
[​](https://trigger.dev/docs/troubleshooting#failed-to-index-deployment-with-column-must-be-greater-than-or-equal-to-0-got-1)

`Failed to index deployment` with `Column must be greater than or equal to 0, got -1`

This can occur when using `runtime: "bun"` during the indexing phase (we load and execute your task files to discover exports). A short-term workaround is to [pnpm patch](https://pnpm.io/cli/patch)
 the `source-map` package. See [this GitHub issue](https://github.com/triggerdotdev/trigger.dev/issues/3045)
 for the patch details.

### 

[​](https://trigger.dev/docs/troubleshooting#reactdomserver-rendertopipeablestream-is-not-a-function-when-using-react-email)
...
[​](https://trigger.dev/docs/troubleshooting#cannot-find-matching-keyid)
...
[​](https://trigger.dev/docs/troubleshooting#project-setup-issues)
...
[​](https://trigger.dev/docs/troubleshooting#the-requested-module-nodeevents-does-not-provide-an-export-named-addabortlistener)
...
[​](https://trigger.dev/docs/troubleshooting#runtime-issues)
...
[​](https://trigger.dev/docs/troubleshooting#environment-variable-not-found)
...
[​](https://trigger.dev/docs/troubleshooting#error-@prisma/client-did-not-initialize-yet)
...
[​](https://trigger.dev/docs/troubleshooting#database-connection-requires-ipv4)
...
In the current version, you can’t perform more that one “wait” in parallel. Waits include:
...
This restriction exists because we suspend the task server after a wait, and resume it when the wait is done. At the moment, if you do more than one wait, the run will never continue when deployed, so we throw this error instead. The most common situation this happens is if you’re using `Promise.all` around some of our wait functions. Instead of doing this use our built-in functions for [triggering tasks](https://trigger.dev/docs/triggering#triggering-from-inside-another-task)
. We have functions that allow you to trigger different tasks in parallel.

### 

[​](https://trigger.dev/docs/troubleshooting#when-triggering-subtasks-the-parent-task-finishes-too-soon)
...
[​](https://trigger.dev/docs/troubleshooting#could_not_find_executor)

`COULD_NOT_FIND_EXECUTOR`

If you see a `COULD_NOT_FIND_EXECUTOR` error when triggering a task, it may be caused by dynamically importing the child task. When tasks are dynamically imported, the executor may not be properly registered. Use a top-level import instead:
...
[​](https://trigger.dev/docs/troubleshooting#rate-limit-exceeded)

Rate limit exceeded

The most common cause of hitting the API rate limit is if you’re calling `trigger()` on a task in a loop, instead of doing this use `batchTrigger()` which will trigger multiple tasks in a single API call. You can have up to 1,000 tasks in a single batch trigger call with SDK 4.3.1+ (500 in prior versions). View the [rate limits](https://trigger.dev/docs/limits)
 page for more information.

### 

[​](https://trigger.dev/docs/troubleshooting#runs-waiting-in-queue-due-to-concurrency-limits)

Runs waiting in queue due to concurrency limits

If runs are staying in the `QUEUED` state for extended periods, check your concurrency usage in the dashboard. Review how many runs are `EXECUTING` or `DEQUEUED` (these count against limits) and check if any runs are stuck in `EXECUTING` state, as they may be blocking new runs. **Solutions:**
...
[​](https://trigger.dev/docs/troubleshooting#crypto-is-not-defined)
...
[​](https://trigger.dev/docs/troubleshooting#task-run-stalled-executing)
...
These errors can happen when code inside your task is blocking the event loop for too long. The most likely cause would be an accidental infinite loop. It could also be a CPU-heavy operation that’s blocking the event loop, like nested loops with very large arrays. If you use **Prisma 7.x**, query compilation and caching run on the main thread and can block the event loop during heavy or repeated database work. In tasks that do a lot of Prisma calls (e.g. in loops or many sequential queries), add `await heartbeats.yield()` periodically so the event loop can run and send heartbeats. We recommend reading the [Don’t Block the Event Loop](https://nodejs.org/en/learn/asynchronous-work/dont-block-the-event-loop)
 guide from Node.js for common patterns that can cause this. If you are doing a continuous CPU-heavy task, then we recommend you try using our `heartbeats.yield` function to automatically yield to the event loop periodically:
...
If the above doesn’t work, then we recommend you try increasing the machine size of your task. See our [machines guide](https://trigger.dev/docs/machines)
 for more information.

### 

[​](https://trigger.dev/docs/troubleshooting#uncaught-exceptions)
...
[​](https://trigger.dev/docs/troubleshooting#realtime-stream-error-sendbatchnonblocking-/-s2appendsession)
...
Alternatively, await `waitUntilComplete()` before closing the stream. See [Realtime Streams](https://trigger.dev/docs/tasks/streams)
 for more.

[​](https://trigger.dev/docs/troubleshooting#framework-specific-issues)
...
[​](https://trigger.dev/docs/troubleshooting#nestjs-swallows-all-errors/exceptions)
...
[​](https://trigger.dev/docs/troubleshooting#react-is-not-defined)
...
[​](https://trigger.dev/docs/troubleshooting#next-js-build-failing-due-to-missing-api-key-in-github-ci)
...
[​](https://trigger.dev/docs/troubleshooting#correctly-passing-event-handlers-to-react-components)
...
[Previous](https://trigger.dev/docs/bulk-actions)
[Database connectionsConnect a database to your tasks: where to create the client, how to size the pool for your provider's connection limit, and how to release connections so you don't run out.\
\
Next](https://trigger.dev/docs/database-connections)
