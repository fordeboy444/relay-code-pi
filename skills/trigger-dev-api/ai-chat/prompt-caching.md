...
> Source: https://trigger.dev/docs/ai-chat/prompt-caching
...
[​](https://trigger.dev/docs/ai-chat/prompt-caching#cache-the-system-prompt)
...
[​](https://trigger.dev/docs/ai-chat/prompt-caching#cache-the-conversation-history)

Cache the conversation history
---------------------------------------------------------------------------------------------------------------------

Place a breakpoint on the last message and the entire conversation prefix up to that point is cached, so the next turn reads it back instead of re-processing it. Do this in [`prepareMessages`](https://trigger.dev/docs/ai-chat/reference#chatagentoptions)
 — it transforms model messages once, and `chat.agent` applies it on every path that builds a prompt (each turn, and both compaction rebuild paths), so the breakpoint always lands on the real last message.
...
The `cacheControl` shorthand is Anthropic-only; `systemProviderOptions` (and `chat.prompt.set`’s `providerOptions`) is the form to reach for on any other breakpoint-based provider. Usage reporting is normalized. Each provider reports cache tokens under its own provider-specific field, but the AI SDK maps them into the same `inputTokenDetails.cacheReadTokens` / `cacheWriteTokens` that `previousTurnUsage` and `totalUsage` carry and the dashboard shows — so the [verify step](https://trigger.dev/docs/ai-chat/prompt-caching#verify-caching-is-working)
 is the same regardless of provider.

[​](https://trigger.dev/docs/ai-chat/prompt-caching#verify-caching-is-working)
...
Anything that changes the prefix between turns silently kills the cache. Keep the system prompt **byte-stable** — never interpolate a timestamp, request ID, or per-turn value into `chat.prompt`. Don’t change the **model** or the **tool set** mid-conversation (tools render at position 0, so adding one invalidates everything after). Inject dynamic per-turn context as a late message via [pending messages](https://trigger.dev/docs/ai-chat/pending-messages)
 or [background injection](https://trigger.dev/docs/ai-chat/background-injection)
, not into the cached prefix.

[​](https://trigger.dev/docs/ai-chat/prompt-caching#next-steps)
...
----------\
\
Keep long conversations within token limits — and re-warm the cache after.](https://trigger.dev/docs/ai-chat/compaction)

[Fast starts\
-----------\
\
Cut cold-start latency so a cached prefix is the only thing between a message and a reply.](https://trigger.dev/docs/ai-chat/fast-starts)

[chat.agent reference\
--------------------\
\
Full option surface, including `prepareMessages` and `toStreamTextOptions`.](https://trigger.dev/docs/ai-chat/reference#chatagentoptions)

[Building agents: backend\
------------------------\
\
The three ways to build a chat backend and when to reach for each.](https://trigger.dev/docs/ai-chat/backend)

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/compaction)
[Pending MessagesInject user messages mid-execution to steer agents between tool-call steps.\
\
Next](https://trigger.dev/docs/ai-chat/pending-messages)
