...
There are three abstraction levels for a chat backend. All three speak the same wire protocol, so the [frontend transport](https://trigger.dev/docs/ai-chat/frontend)
 works unchanged whichever you pick.
...
The raw-primitives column assumes [`chat.customAgent()`](https://trigger.dev/docs/ai-chat/custom-agents)
...
[​](https://trigger.dev/docs/ai-chat/backend#chat-agent)
...
[​](https://trigger.dev/docs/ai-chat/backend#simple-return-a-streamtextresult)
...
**Always spread `chat.toStreamTextOptions()` first** (as above) so your explicit overrides win. It wires up the `prepareStep` callback behind [compaction](https://trigger.dev/docs/ai-chat/compaction)
, [steering](https://trigger.dev/docs/ai-chat/pending-messages)
, and [background injection](https://trigger.dev/docs/ai-chat/background-injection)
, all of which silently no-op without it, and injects the system prompt from `chat.prompt()`, the resolved model (when you pass a `registry`), and telemetry metadata. Examples below keep the spread implicit for brevity, so include it in real code.

### 

[​](https://trigger.dev/docs/ai-chat/backend#using-chat-pipe-for-complex-flows)
...
[​](https://trigger.dev/docs/ai-chat/backend#custom-data-parts)
...
`chat.response` and the `writer` accumulation behavior work with `chat.agent` and `chat.createSession`. If you’re using [`chat.customAgent`](https://trigger.dev/docs/ai-chat/custom-agents)
, you own the accumulator — see the raw-task example for the manual pattern.

### 

[​](https://trigger.dev/docs/ai-chat/backend#raw-streaming-with-chat-stream)
...
See [Tools](https://trigger.dev/docs/ai-chat/tools)
 for `toModelOutput` across turns, per-turn dynamic tools, the typed run payload, and how config tools relate to skills.

### 

[​](https://trigger.dev/docs/ai-chat/backend#lifecycle-hooks)
...
*   [Lifecycle hooks](https://trigger.dev/docs/ai-chat/lifecycle-hooks)
     — `onPreload`, `onChatStart`, `onValidateMessages`, `hydrateMessages`, `onTurnStart`, `onBeforeTurnComplete`, `onTurnComplete`, `onChatSuspend` / `onChatResume`, `exitAfterPreloadIdle`, plus how `ctx` plumbs through every callback.
...
[​](https://trigger.dev/docs/ai-chat/backend#using-prompts)

Using prompts

Use [AI Prompts](https://trigger.dev/docs/ai/prompts)
 to manage your system prompt as versioned, overridable config. Store the resolved prompt in a lifecycle hook with `chat.prompt.set()`, then spread `chat.toStreamTextOptions()` into `streamText` — it includes the system prompt, model, config, and telemetry automatically.
...
| `chat.toStreamTextOptions({ registry })` | You’re using [Prompts](https://trigger.dev/docs/ai/prompts)<br> with a provider-prefixed model string (e.g. `"anthropic:claude-sonnet-4-5"`). The registry resolves the prefix to a real model instance via `createProviderRegistry({ anthropic, openai, ... })`. |
...
See [Prompts](https://trigger.dev/docs/ai/prompts)
 for the full guide — defining templates, variable schemas, dashboard overrides, and the management SDK.

### 

[​](https://trigger.dev/docs/ai-chat/backend#stop-generation)

Stop generation

#### 

[​](https://trigger.dev/docs/ai-chat/backend#how-stop-works)
...
[​](https://trigger.dev/docs/ai-chat/backend#abort-signals)
...
[​](https://trigger.dev/docs/ai-chat/backend#detecting-stop-in-callbacks)
...
[​](https://trigger.dev/docs/ai-chat/backend#cleaning-up-aborted-messages)
...
[​](https://trigger.dev/docs/ai-chat/backend#tool-approvals)
...
[​](https://trigger.dev/docs/ai-chat/backend#pending-messages-steering)
...
See [Pending Messages](https://trigger.dev/docs/ai-chat/pending-messages)
 for the full guide — backend configuration, frontend hook, queuing vs steering, and how injection works with all three chat variants.

### 

[​](https://trigger.dev/docs/ai-chat/backend#background-injection)
...
See [Background Injection](https://trigger.dev/docs/ai-chat/background-injection)
 for the full guide — timing, self-review example, and how it differs from pending messages.

### 

[​](https://trigger.dev/docs/ai-chat/backend#actions)

Actions

Custom actions let the frontend send structured commands (undo, rollback, edit, regenerate) that modify the conversation state. **Actions are not turns**: they fire `hydrateMessages` (if set) and `onAction` only. The full surface (defining `actionSchema`, returning a model response from `onAction`, gating against pending HITL tool calls, and sending actions from the frontend) lives on its own page. See [Actions](https://trigger.dev/docs/ai-chat/actions)
.

### 

[​](https://trigger.dev/docs/ai-chat/backend#chat-history)
...
The agent’s accumulator — not `session.out` — is the source of truth for the full conversation. The `.out` stream is a bounded sliding window (roughly one turn at steady state, see [Records on `session.out`](https://trigger.dev/docs/ai-chat/client-protocol#records-on-session-out)
); the durable history lives in the agent’s accumulator and is persisted to S3 between turns for fast next-run boots. `chat.history` reads and mutates that accumulator directly.
...
`extractNewToolResults` compares against the _current_ chain. Inside `onTurnComplete`, the chain already contains the just-finished `responseMessage`, so it returns `[]`. Use it where the message is from outside the accumulator: `hydrateMessages` (incoming wire), `onAction` if the action carries a message, or any custom pre-merge code path.

### 

[​](https://trigger.dev/docs/ai-chat/backend#preparemessages)
...
[​](https://trigger.dev/docs/ai-chat/backend#version-upgrades)

Version upgrades

Chat agent runs are pinned to the worker version they started on. When you deploy a new version, suspended runs resume on the old code. Call `chat.requestUpgrade()` in `onTurnStart` to skip `run()` and exit immediately — the transport re-triggers the same message on the latest version. See the [Version Upgrades pattern](https://trigger.dev/docs/ai-chat/patterns/version-upgrades)
 for the full guide.

### 

[​](https://trigger.dev/docs/ai-chat/backend#ending-a-run-on-your-terms)
...
[​](https://trigger.dev/docs/ai-chat/backend#runtime-configuration)

Runtime configuration

#### 

[​](https://trigger.dev/docs/ai-chat/backend#chat-setturntimeout)
...
[​](https://trigger.dev/docs/ai-chat/backend#chat-setidletimeoutinseconds)
...
[​](https://trigger.dev/docs/ai-chat/backend#stream-options)
...
`chat.setUIMessageStreamOptions()` works across all abstraction levels — `chat.agent()`, `chat.createSession()` / `turn.complete()`, and `chat.pipeAndCapture()`. See [ChatUIMessageStreamOptions](https://trigger.dev/docs/ai-chat/reference#chatuimessagestreamoptions)
 for the full reference.

`onFinish` is managed internally for response capture and cannot be overridden here. Use `streamText`’s `onFinish` callback for custom finish handling, or use [raw task mode](https://trigger.dev/docs/ai-chat/custom-agents)
 for full control over `toUIMessageStream()`.

### 

[​](https://trigger.dev/docs/ai-chat/backend#manual-mode-with-task)
...
[​](https://trigger.dev/docs/ai-chat/backend#custom-agents)
...
[Previous](https://trigger.dev/docs/ai-chat/anatomy)
[Lifecycle hooksHook into every stage of a chat agent's run: preload, turn start, turn complete, suspend, resume, and more.\
\
Next](https://trigger.dev/docs/ai-chat/lifecycle-hooks)
