...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#conceptual-data-model)
...
Store **`UIMessage[]`** in a JSON-compatible column, or normalize to a messages table — the pattern is _when_ you read/write, not _how_ you encode rows.

[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#where-each-hook-writes)

Where each hook writes
--------------------------------------------------------------------------------------------------------------------

This pattern covers **durable DB rows** (the conversation and the active session). Per-process in-memory state ([`chat.local`](https://trigger.dev/docs/ai-chat/chat-local)
, [DB connection pools](https://trigger.dev/docs/database-connections)
, sandboxes, etc.) belongs in [`onBoot`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#onboot)
 — it fires on every fresh worker including continuation runs, where `onPreload` and `onChatStart` do not.

### 

[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#onpreload-optional)

`onPreload` (optional)

When the user triggers [preload](https://trigger.dev/docs/ai-chat/fast-starts#preload)
, the run starts **before** the first user message.
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#onchatstart-chat%E2%80%99s-first-message-non-preloaded-path)
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#onturnstart)
...
**Don’t use [`chat.defer()`](https://trigger.dev/docs/ai-chat/background-injection#chat-defer-standalone)
 for the message write here.** `chat.defer` is fire-and-forget — the hook resolves before the write lands and the stream starts immediately. If the user refreshes mid-stream, the next page load reads `[]` from your DB, the resumed SSE stream pushes the assistant into an empty array, and the user’s message disappears from the rendered conversation forever.
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#onturncomplete)
...
**`lastEventId`** lets the frontend [resume](https://trigger.dev/docs/ai-chat/frontend)
 without replaying SSE events it already applied. Treat it as part of session state, not optional polish, if you care about duplicate chunks after refresh.
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#token-renewal-app-server)
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#minimal-pseudocode)
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#alternative-hydratemessages)

Alternative: `hydrateMessages`
---------------------------------------------------------------------------------------------------------------------------------

For apps that need the backend to be the single source of truth for message history — abuse prevention, branching conversations, or rollback support — use [`hydrateMessages`](https://trigger.dev/docs/ai-chat/lifecycle-hooks#hydratemessages)
...
This replaces the `onTurnStart` persistence pattern — the hook handles both loading and persisting the new message in one place. Hydration composes with [Head Start](https://trigger.dev/docs/ai-chat/fast-starts#with-hydratemessages)
: on a head-start first turn the route handler’s history arrives as `incomingMessages`, and the write path must be an upsert because no preload ran to create the row.

[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#design-notes)
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#complete-example)
...
[​](https://trigger.dev/docs/ai-chat/patterns/database-persistence#see-also)
...
*   [Session management](https://trigger.dev/docs/ai-chat/frontend#session-management)
     — `resume`, `lastEventId`, transport
*   [`chat.defer()`](https://trigger.dev/docs/ai-chat/background-injection#chat-defer-standalone)
     — non-blocking writes during a turn
*   [Code execution sandbox](https://trigger.dev/docs/ai-chat/patterns/code-sandbox)
     — combines **`onWait`** / **`onComplete`** with this persistence model

Was this page helpful?

YesNo

[Previous](https://trigger.dev/docs/ai-chat/patterns/version-upgrades)
[Persistence and replayHow chat.agent rebuilds conversation history at run boot — durable JSON snapshot in object storage plus session.out replay, with a hydrateMessages short-circuit for backend-owned history.\
\
Next](https://trigger.dev/docs/ai-chat/patterns/persistence-and-replay)
