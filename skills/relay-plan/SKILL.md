---
name: relay-plan
description: Use when a design spec already exists in docs/specs/ and the user wants a step-by-step implementation plan before any code is written. Triggers on "write the plan", "plan the implementation", or /relay-plan <slug>. Do not use to design the automation itself — use /relay-brainstorm for that.
---
...
This is the second phase of the three-skill flow: **brainstorm → plan → execute**. It consumes the spec (the *what*/*why*, from `/relay-brainstorm`) and produces the plan (the *how*). `/relay-execute` then builds it.

## Input

Pass a slug: `/relay-plan <slug>`. If no slug is given, list `docs/specs/*.md` and use the **AskUserQuestion tool** (rpiv-ask-user-question) to let the user pick.

Read the spec at `docs/specs/<slug>.md`. If no spec exists for the slug, stop and tell the user to run `/relay-brainstorm` first — this skill does **not** design automations.

## Workflow

0. **Locate the automation.** Call **`relay_locate_automation`** with the slug (or without one to list candidates, then ask the user to pick). Confirm the spec exists; if not, stop and tell the user to run `/relay-brainstorm` first. If a plan for this slug already exists, ask whether to overwrite, create a new dated version, or open the existing one.
...
4. **Confirm the sub-agent roster.**
   - Default to the agents named in the spec's `## Sub-agents` section. Primary agents (`modal-agent`, `trigger-dev-agent`, `Explore`) are assumed unless the spec omits them; add-on agents appear only when the spec adds them.
...
5. **Right-size the tasks.**
   ...
   - **One domain agent per task by default.** If a task genuinely needs 2+ agents, list them in sequential dependency order (`Agents: airtable-agent → trigger-dev-agent`); `/relay-execute` runs them sequentially and reviews the combined diff once.
...
7. **Write the plan** to `docs/plans/YYYY-MM-DD-<slug>.md` using this structure:
   ...
   > **For agentic workers:** REQUIRED SUB-SKILL: `/relay-execute` implements this plan task-by-task via `pi-subagents`. Steps use `- [ ]` checkboxes for tracking; the live todo overlay (rpiv-todo) mirrors the progress ledger.
...
10. **Self-check conformance, then hand off.** Once approved, set plan `status: planned`. Call **`relay_lint`** and confirm it is clean — it checks the plan's `status` frontmatter, that every `**Agents:**` line names only the six ported agents, and that the spec has `slug`/`name`/`trigger_type`/`created` frontmatter. Fix every error it reports (edit the named file), then re-call `relay_lint` until clean. Do **not** auto-invoke the next skill. Stop and tell the user:
    > The plan is written to `docs/plans/YYYY-MM-DD-<slug>.md`. Run `/clear` to clear the context, then run `/relay-execute <slug>` to start (or resume) the implementation. Execute writes and tests the Trigger.dev task against the **local dev worker** (it calls `relay_dev_worker up` itself); when all tasks are done, execute deploys to production via `relay_deploy_trigger` → `relay_smoke_test` → `relay_deploy_modal`.
...
- **Plan-only:** never write the spec here — that is `/relay-brainstorm`'s job. If no spec exists, refuse and point the user there.
- Never write application code (`src/*`, `modal_bridge.py`, `scripts/*`) in this skill — delegate to the agents during `/relay-execute`. The plan may *instruct* calling `relay_add_*` tools but does not call them itself.
...
- Always pause for explicit user approval (via @plannotator) before handing off to `/relay-execute`.
