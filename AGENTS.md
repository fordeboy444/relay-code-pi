...
`relay-code-pi` is a **Pi coding-agent package** â€” an extension bundle installed into the
...
Before any task, load the **`pi-api` skill** (`.agents/skills/pi-api/SKILL.md`) â€” no
...
npm test               # vitest run â€” pure-core unit tests (the primary gate; TS pipeline exercised here)
npm run test:watch     # vitest watch
pi install . -l        # set up this repo as a project-local Pi package (writes .pi/settings.json)
pi -e .               # package-load smoke gate â€” expect SMOKE_OK (loads 11 relay tools + 8 extensions)
...
`tsconfig.json` type-checks `src/**/*.ts` + `tests/**/*.ts` only â€” **`extensions/` is excluded**
...
- **`src/cores.ts`** â€” pure, side-effect-free functions. They take file **content** (strings) +
...
- **`extensions/relay-tools.ts`** â€” one file, one default export factory, many
  `pi.registerTool` calls. Each tool's `execute` body is **thin glue**: a file mutation is
  one `mutateFile(path, core)` call â€” the shared adapter that runs read â†’ pure core â†’ write
  inside `withFileMutationQueue`, so the queue is never skipped â€” or it shells out via
...
breaks testability â€” the extension can't be unit-tested without a running Pi host.
...
The `package.json` `"pi"` block is the manifest Pi reads â€” `extensions`, `skills`, `prompts`,
...
tool-use rules, deploy order, and security invariants â€” Pi auto-loads it into the system
...
developer's session). The constitution is no longer per-project scaffolded â€” Pi injects it
...
All `relay_add_*` tools are **idempotent** â€” re-running with the same params is a no-op, and the
pure cores detect "already present" before mutating. They throw on a **missing anchor** (e.g.
`export const config = {`, `ALLOWED_TASKS: set[str] = â€¦`) rather than silently corrupting a
hand-edited file. A tool reporting a missing anchor means the project file was hand-edited away
from the framework template â€” surface this to the user; do not patch it by hand.
...
Two of those templates are dotfiles â€” `.gitignore`, `.env` â€” that **cannot ship under their bare
...
The `cp -r templates/. <root>` shortcut does **not** handle these two â€” they need a name-remapping
Readâ†’Write. Do not "fix" this by renaming them back to bare names or removing the ignore rules â€”
...
- `trigger.config.ts` ships with `maxDuration: 300` (5 minutes â€” a safe default
  for a starter automation). Trigger.dev CLI â‰¥ 4.5 requires this option; do not
...
Pi loads a project's context as `AGENTS.override.md` â†’ `AGENTS.md` â†’ `CLAUDE.md` (default
`AGENTS.md`). `relay_add_env_var` writes its env-table row into whichever of these exists
(`cores.pickContextFile`). A scaffolded relay-code project ships **no** root context file â€”
the constitution lives in `prompts/AGENTS.md` (system-prompt injection, not a context file) â€”
...
auto-edit it). `.env.production` is no longer scaffolded â€” the user owns that file (load via
...
- **Keep cores pure.** New transform â†’ `src/cores.ts` + a fixture-backed test in
...
- **Windows is a first-class host** (`process.platform === "win32"` paths exist in the
  ...
  remove them.
- **Secrets handling is governed by the constitution** â€” see `prompts/AGENTS.md`
  ...
  only the prefix (`tr_dev_â€¦`, `tr_prod_â€¦`, `ak-â€¦`, `as-â€¦`).
- The `package.json` `"pi"` block is the manifest Pi reads â€” `extensions`, `skills`, `prompts`,
  `subagents` arrays. Adding a new extension/skill/agent means adding its path here.

## Key references in-repo

- `prompts/AGENTS.md` â€” the constitution (repo root, also shipped in the npm tarball);
...
**Auth â€” granular access token with Bypass-2FA.** The maintainer account has 2FA enabled, so
...
npm writes the line to `~/.npmrc` silently â€” `npm config get` returns `protected` rather than
the value, by design. **Revoke the token on https://www.npmjs.com â†’ Access Tokens when you
stop using it** â€” Bypass-2FA tokens are high-value credentials. Note: Bypass-2FA tokens **lose
...
1. Bump `version` in `package.json` by hand (never `npm version` on a Pi package â€” keep the
...
2. Update the README **Version history** table â€” append a row for the new version.
3. Commit, tag, and push the release on `main` â€” **every npm version must exist on GitHub**.
   A version published with no matching git commit is unrecoverable drift (see the
   0.5.1/0.5.2/0.5.5 rows in the README):
   ```sh
   git add package.json README.md    # add ONLY the release files â€” never `git add -A`
   ...
   early; the required order is commit â†’ tag â†’ push â†’ dry-run â†’ publish.)
4. `npm pack --dry-run` â€” confirm only the intended files ship. For relay-code-pi that means
   ...
   `@juicesharp/rpiv-todo`, `@llblab/pi-telegram` â€” see the bundle note below). The `npm pack`
   ...
   allows (`skills/` + `package.json` + `README.md`). **Never** ship real `.env` files â€”
   accidental `npm publish` of a secrets-bearing `.env` is unrecoverable.
5. `npm publish`.
6. Verify with `npm view <package>` â€” there is a registry propagation lag after publish.
   ...
   ~6.7k files, own + bundled `node_modules/`) it took **25 minutes** in observed practice â€” plan for 10-30 minutes before
   ...
   `http fetch PUT 202` + `verbose exit 0` + `info ok` â€” that's the success signal; the
   long wait is CDN/index propagation, not a publish failure. **Do NOT re-run `npm publish`
   while waiting** â€” the version is committed server-side and a retry will fail with
...
- `relay-code-pi`: `extensions/`, `src/`, `skills/`, `agents/`, `prompts/`, `package.json`,
  ...
  (see the bundle note below). `src/` must ship â€” `extensions/relay-tools.ts` imports
...
(`node_modules/@llblab/pi-telegram/index.ts`, `node_modules/pi-subagents/skills`, â€¦). Pi loads
...
invisible to Pi â€” its extension + skills silently fail to load (this is why `@llblab/pi-telegram`
...
**Spelling gotcha â€” use `bundleDependencies` (no "d").** The older `bundledDependencies`
spelling is **deprecated**: npm 9 warns, npm 10 **throws**. (That deprecation noise is what
prompted 0.5.3 to delete the field entirely â€” the wrong cure; the right one was to rename it.)
Keep the field spelled `bundleDependencies` and keep all 6 packages listed whenever you add or
remove a `node_modules/<pkg>` reference from the `pi` manifest â€” a referenced-but-not-bundled
...
masters at `~/.pi/agent/skills/env-storage/` are never touched** â€” Pi's package install only
