...
- `relay_deploy_modal` — `modal deploy modal_bridge.py`. **Refuses until
  ...
  before Trigger.dev.
- `relay_setup` — fill the `REPLACE-ME` project identity sentinels on first run.
...
- Phase 1 brainstorm → `docs/specs/<slug>.md` (no code). Phase 2 plan →
  ...
  `gohighlevel-agent`, `unipile-agent`, `apify-agent`, or `Explore`). Phase 3 execute
  → `.claude/automation/<plan>/progress.md`. Each phase writes its artifact and stops.
